...
const mainNav: NavItem[] = [
  ...
  { to: '/servicos', label: 'Cortes & Serviços', icon: Scissors },
...
const salesNav: NavItem[] = [
  { to: '/leads', label: 'Leads', icon: Target, amber: true },
  { to: '/cadencia', label: 'Cadência', icon: Zap, amber: true },
  { to: '/metas', label: 'Metas', icon: Trophy, amber: true },
]
...
function NavGroup({ items }: { items: NavItem[] }) {
  return (
    <ul className="space-y-0.5">
      {items.map(({ to, label, icon: Icon, amber }) => (
        <li key={to}>
          <NavLink
            to={to}
            end={to === '/'}
            ...
            {({ isActive }) => (
              <>
                <Icon
                  className={[
                    'h-4 w-4 shrink-0',
                    isActive ? 'text-white' : amber ? 'text-amber-400' : 'text-slate-400',
                  ].join(' ')}
                />
                <span className={!isActive && amber ? 'text-amber-400' : undefined}>
                  {label}
                </span>
              </>
            )}
...
function SidebarContent({ onClose }: { onClose?: () => void }) {
  const { signOut, barbeariaId } = useAuth()
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-5 border-b border-slate-700">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-white text-lg tracking-tight">BarbeiroPro</span>
        </div>
        {onClose !== undefined ? (
          <button
            onClick={onClose}
