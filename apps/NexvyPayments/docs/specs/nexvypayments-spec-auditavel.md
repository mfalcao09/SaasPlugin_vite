...
| A2 | Arquivo `_shared/require-caller-org.ts` + suite de teste com os 3 casos (403/401/org-do-token) passando | **CONFORME** — 2026-07-06, iteração 5: `supabase/functions/_shared/require-caller-org.ts` + `_shared/__tests__/require-caller-org.test.ts`; `deno test` = **11 passed, 0 failed** (3 casos do spec + service_role/actorUserId + 7 robustez), aferição re-executada pelo revisor (autor: subagente Opus). Org real via `profiles.organization_id` (padrão do baseline: webchat-inbox/index.ts:93). Nota: comparação da service key é `===` (paridade com molde platform-crm-auth) — endurecer p/ timing-safe no consumo (Fases B+/A6) |
| A3 | `docs/security/rls-audit-2026-07.md` com query + resultado: N tabelas, 100% ON, lista de policies permissivas com justificativa/correção | **PROXY_PRONTO** — 2026-07-06, iteração 6: doc + par .html em `docs/security/`; Q1–Q3 versionadas; **112/112 tabelas com organization_id RLS ON**, 0 sem RLS, 0 deny-all acidental, 415 policies, **20 permissivas classificadas** (17 service_role justificadas · 2 captação pública · 1 catálogo · 🚩 1 flag `help_article_feedback` p/ decisão do revisor). Certificação → G-SEC-REV (P3) |
| A4 | Migration aplicada (`migrations_cobranca/*_billing_credentials.sql`) + teste: SELECT anon = 0 linhas + round-trip `v1:` passando (reuso `meta-crypto.ts:25,41`) | **CONFORME** — 2026-07-06, iteração 8: migration aplicada no Supabase `nbvaglqmcyoogolhzyzm` (`supabase db query --linked`); `_shared/billing-crypto.ts` (wrapper fino sobre `meta-crypto.ts:25,41`) + suite `__tests__/billing-crypto.test.ts` = **8 passed, 0 failed** (`deno test --no-check`: round-trip `v1:`, IV aleatório, falhas controladas, prova de zero-rede). Segurança verificada no banco: `SET ROLE anon; SELECT` = **0 linhas**; `rls_on=true`. **Endurecimento (defesa em profundidade)**: default privilege do schema `public` vazava SELECT a anon/authenticated — adicionado `REVOKE ALL FROM anon, authenticated` → `has_table_privilege('anon')=false`, `('authenticated')=false`, só `service_role=true` |
| A5 | Migrations `migrations_cobranca/` aplicadas; `\d invoices` com as 9 colunas novas; CHECK de status com `substituida`; `\d billing_credentials` com `cnpj` + UNIQUE triplo; `\d billing_agreements` existe; toda tabela org-scoped; teste INSERT cross-org rejeitado | **CONFORME** — 2026-07-06, iteração 9: `20260706101000_billing_model.sql` aplicada no banco (7 tabelas: payers, billing_groups, contracts, billing_agreements, invoices, invoice_items, billing_events). Aferido no banco: **7/7 RLS ON**; `invoices` com as 8 colunas de correção (multa_pct/juros_pct/valor_multa/valor_juros/valor_original/agreement_id/iss_retido/retencoes); CHECK de status inclui `substituida`; **7/7 tabelas com organization_id**; 5 policies em invoices; **INSERT cross-org como authenticated → `ERROR 42501 new row violates row-level security policy`**. Isolamento: 0 ALTER/DROP de core, REFERENCES só a `organizations`/`leads`. `cnpj`+UNIQUE triplo já em A4 (billing_credentials) |
| A6 | `docs/security/verify-jwt-matrix.md` com outputs de curl: 2 webhooks sem JWT ≠ 401; 3 funções de dinheiro sem JWT = 401 | **CONFORME** — 2026-07-06, iteração 12: 6 EFs deployadas no `nbvaglqmcyoogolhzyzm` (ACTIVE); `docs/security/verify-jwt-matrix.md` com curl externo real: `notaas-webhook` (público) → **404 ≠ 401** (gateway passou); `billing-baixa-manual` + `invoice-batch-generate` (dinheiro) → **401**. Mecanismo verify_jwt provado. Ressalva: `c6-webhook`/`c6-billing`/`notaas-emit` (gates C6/NotaAS) herdam o padrão, entram na matriz ao serem criadas |
| A7 | grep: 0 tabelas de cobrança fora de `migrations_cobranca/`; `docs/CORE-DELTA.md` existe e lista toda edição a arquivo do core (com justificativa); nenhuma migration de cobrança faz `ALTER`/`DROP` em tabela do core sem entrada em CORE-DELTA | **CONFORME** — 2026-07-06, iteração 7 (invariante contínuo, re-aferido por fase): grep tabelas de cobrança fora da esteira = **0**; `CORE-DELTA.md` com 7 entradas (5 deltas commit `22170d1` + 2 não-edições auditadas); 0 ALTER/DR0P de core em `migrations_cobranca/` (esteira criada com README de disciplina). Cert. revisor → G-SEC-REV (P3) |
...
| B2 | `SELECT pgmq.metrics('billing_outbox')` OK; testes das RPCs enqueue/read/dlq verdes | **CONFORME** — 2026-07-06, iteração 10: `20260706102000_billing_outbox.sql` aplicada (pgmq.create + RPCs `billing_outbox_enqueue`/`_read_batch`/`_move_to_dlq` + DLQ). Aferido no banco (BEGIN/ROLLBACK): enqueue→read_batch(traz a `referencia`)→move_to_dlq → **na_dlq=1**. GRANT EXECUTE só a service_role. Molde: wrappers de e-mail pgmq do Beauty (`process-email-queue`) |
| B3 | Teste: 2 execuções mesma competência → count igual; teste dia útil (vencimento cai sábado → segunda) | **CONFORME** — 2026-07-06, iteração 11: `invoice-batch-generate/index.ts` (idempotente: UNIQUE(org,contract,competencia) + captura SQLSTATE 23505 como skip; dry_run default) + migration `billing_next_business_day`/`billing_holidays` org-scoped. deno test 12/12; **banco**: `billing_next_business_day(NULL,'2026-05-09'[sáb])='2026-05-11'[seg]`. Nota: não usei `platform_crm_business_holidays` (global/sem org) — criei `billing_holidays` org-scoped |
...
| C2 | Testes: HMAC inválido → 401; deliveryId repetido → 1 processamento; `documents_ready` partial+complete → estado final correto | **CONFORME** — 2026-07-06, iteração 10: `notaas-webhook/index.ts` + `_shared/notaas-webhook-verify.ts` (HMAC-SHA256 timing-safe) + **deno test 14/14**; migration `notaas_webhook_deliveries` (ledger dedup) aplicada — `ledger_ok=true`, UNIQUE(org,delivery_id), RLS ON; `invoices.nfse_status` já existia (billing_model). `config.toml [functions.notaas-webhook] verify_jwt=false`. Dedup atômico via UPSERT ignoreDuplicates |
| C3 | Teste: DELETE em nota `issued` → exceção do guard; fluxo de cancelamento gera evento com trilha | **CONFORME** — 2026-07-06, iteração 10: `20260706103000_fiscal_imutabilidade.sql` aplicada (trigger `guard_invoice_fiscal_immutability` BEFORE DELETE/UPDATE + função `invoice_cancelar`). Aferido no banco (fixture org/payer/invoice `emitida`, ROLLBACK): DELETE de nota emitida → **bloqueado**; `invoice_cancelar` → status=`cancelada` + `billing_events(cancelada)`=1. Cancelamento via GUC de sessão (única mutação permitida) |
| D1 | Teste: `billing_events(emitida)` → enrollment com `invoice_id`; render da 1ª msg contém valor+vencimento da fatura | **CONFORME** — 2026-07-06, iteração 11: `billing-cadence-enroll/index.ts` + `_shared/billing-cadence.ts` (`renderFirstMessageContext`) + migration `billing_cadence_enrollments` (banco: `enroll_ok`, RLS ON, `invoice_id` presente); deno test verde. Enrollment keyed por `invoice_id`; 1ª msg cita fatura/valor/vencimento. Molde contrastado: `cadence-enroll` (lead-cêntrico) do Beauty |
| D2 | Teste com clock fixo (TZ São Paulo): fatura venc D+3 → step D-3 hoje; D0 e D+7 nas datas certas | **CONFORME** — 2026-07-06, iteração 11: `computeScheduledAtByDueDate` (steps D-3/D0/D+1/D+7 relativos ao `invoices.vencimento`, NÃO now+delay; TZ America/Sao_Paulo -03:00, sem `new Date()` — lição iso-date-format-br); deno test com clock fixo verde. Contraste: `cadence-tick:34-40` (delay-based) do Beauty |
| D3 | Teste: payer com 2 faturas, paga a 1ª → step_runs da 1ª cancelados, da 2ª intactos | **CONFORME** — 2026-07-06, iteração 11: `billing-cadence-stop/index.ts` keyed por `(enrollment, invoice_id)` — NÃO por lead_id (a correção-chave do adversarial); UNIQUE parcial "1 régua ativa por fatura" + índice `idx_bcsr_invoice_scheduled` (stop O(1)). deno test prova: payer com 2 faturas paga 1 → régua da paga PARA, da outra CONTINUA. Contraste: `cadence-stop:19` (por-lead) do Beauty. Gatilho: `billing_events(paga)` |
...
| D5 | Testes das 4 tools: 2ª via cria nova invoice + original `substituida`; renegociar 3× → 1 agreement + 3 parcelas; desconto>alçada → handoff + 0 faturas; injeção >8k → bloqueado | **CONFORME** — 2026-07-06, iteração 10: `_shared/tools/impl/{consultar_fatura,segunda_via,enviar_comprovante,renegociar}.ts` + `_shared/prompt-guard.ts` + registro aditivo em `registry.ts` (padrão Beauty registry.ts:5-16, assinaturas públicas intactas → webchat-bot não quebra); **deno test 22/22** (segunda_via→nova invoice+original `substituida`; renegociar→1 agreement+N parcelas c/ agreement_id; desconto>alçada→handoff+0 faturas; injeção/>8k→bloqueado+hash). Nota: alçada 20% hardcoded (`MAX_DESCONTO_ALCADA_PCT`) → por-org depois (P-COBR) |
| E1 | Teste: baixa manual grava evento `{origem:'manual'}`; payer sem whatsapp → mensagem na fila pgmq de e-mail (`process-email-queue`) | **CONFORME** — 2026-07-06, iteração 11: `_shared/billing-baixa.ts` + `billing-baixa-manual/index.ts` (requireCallerOrg anti-IDOR) + RPC `invoice_baixa_manual` (respeita trigger C3: emitida/vencida→paga) + `_shared/billing-notify.ts` (fallback e-mail via `enqueue_email` pgmq). **banco** (fixture, ROLLBACK): baixa → `billing_events(paga, origem=manual)`=1. deno test 11/11 (baixa + fallback e-mail para payer sem whatsapp) |
| E2 | Teste: CRUD payers → linhas em `platform_audit_logs`; erasure → contato anonimizado E fatura/nota intactas | **CONFORME** — 2026-07-06, iteração 10: `20260706104000_lgpd_payers.sql` aplicada (trigger `audit_payers_pii` INSERT/UPDATE/DELETE→`platform_audit_logs` sem PII em claro; `payer_erasure` anonimiza contato mantendo notas; `lgpd_legal_basis` default `execucao_contrato`). Aferido no banco (fixture, ROLLBACK): erasure → nome=`[removido]`, **invoice intacta=1**, audit_logs=2. **Bug corrigido na aferição**: trigger inseria `entity_id` como text; coluna é uuid → `ERROR 42804` → removido o cast (`v_entity` já é uuid). `extensions.digest` qualificado (evita 42883). Molde: `migrations_salao/20260619_lgpd_consents.sql` |
