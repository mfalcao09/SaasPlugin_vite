...
import { usePlatformCrmStages } from '@/components/superadmin/crm/data/usePlatformCrmStages';
import { PlatformCrmOptionActionsEditor } from './PlatformCrmOptionActionsEditor';

/**
 * Porte de admin/forms/FormBlockEditor.tsx (núcleo ~50KB).
 * Adaptações:
 *  - useLeadTags → usePlatformCrmTags (retorna {id,name,color})
 *  - useProductPipelineStages(product_id) → usePlatformCrmStages(product_id)
 *  - useAuth REMOVIDO. Upload de mídia usa bucket `form-media` com path
 *    `platform/{form_id}/{uuid}.{ext}` (sem organization_id).
 *    NOTA: o bucket `form-media` + RLS pode ser um asset do tenant — a plataforma
 *    talvez precise de bucket/policy próprios. Sinalizado no relatório.
 *  - OptionActionsEditor → PlatformCrmOptionActionsEditor (recebe productId do form).
 */
...
interface PlatformCrmFormBlockEditorProps {
...
export function PlatformCrmFormBlockEditor({
  block,
  allBlocks,
  form,
  onUpdate,
  onClose,
}: PlatformCrmFormBlockEditorProps) {
  ...
  const { data: pipelineStages } = usePlatformCrmStages(form?.product_id);
  ...
  const handleOptionsChange = (options: SelectOption[] | ScaleOptions) => {
    handleChange('options', options as FormBlock['options']);
  };
  ...
  // Tags management (block.apply_tags — string[])
  ...
  const uploadToBucket = async (file: File): Promise<string | null> => {
    // Sem organization_id: path prefixado por `platform` no bucket `form-media`.
    const ext = file.name.split('.').pop() || 'bin';
    const path = `platform/${localBlock.form_id}/${crypto.randomUUID()}.${ext}`;
    ...
    <div className="w-full md:w-80 bg-card md:border-l flex flex-col h-full">
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-5">
          ...
          {!['welcome_screen', 'end_screen', 'conditional', 'score', 'tag', 'hidden_field'].includes(
            localBlock.block_type,
          ) && (
            <>
              <Separator />
            ...
            <>
              ...
              <div className="space-y-3">
                  ...
                  {((localBlock.options as SelectOption[]) || []).map((option, index) => (
                    <div key={index} className="flex items-center gap-1.5 bg-muted rounded-lg p-2">
                      ...
                      <div
                        ...
                        title="Pontos somados ao score do lead"
                      >
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            updateOption(index, {
                              score: raw === '' ? undefined : parseInt(raw, 10) || 0,
                            });
                          }}
                          placeholder="0"
                      ...
                      <PlatformCrmOptionActionsEditor
                ...
                <p className="text-[10px] text-muted-foreground leading-tight">
                  Use o ícone ⚡ ao lado de cada opção para configurar ações como redirecionar para
                  pagamento, abrir o calendário, atribuir setor/agente, ou adicionar etiquetas. O
                  campo <strong>pts</strong> soma pontos ao score do lead quando a opção é escolhida
                  (classifica em frio/morno/quente).
                </p>
              </div>
            </>
          )}
            ...
            <>
              ...
              <div className="space-y-2">
                <Label className="flex items-center gap-1.5">
                  <Target className="w-3.5 h-3.5" /> Pontuação por resposta
                </Label>
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="10"
                />
                <p className="text-xs text-muted-foreground">Pontos adicionados ao score do lead</p>
              </div>
            </>
          )}
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => handleChange('maps_to', value === 'none' ? undefined : value)}
                >
                  <SelectTrigger>
              ...
              <div className="space-y-4 rounded-lg border border-primary/20 bg-primary/5 p-3">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {crm.add_tag_ids.map((id) => {
                        ...
                        return (
                          ...
                          >
                            {t.name}
                            <button onClick={() => toggleCrmTag(id)}>
                              <X className="w-3 h-3" />
                            </button>
                          </Badge>
                        );
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      updateCrm({ set_temperature: v === 'none' ? null : (v as LeadTemperature) })
                    }
                  >
                    <SelectTrigger className="h-8">
                    ...
                    <div key={rule.id || idx} className="rounded border bg-background p-2 space-y-2">
                      <div className="flex items-start gap-1.5">
                        <div className="flex-1 grid grid-cols-2 gap-1.5">
                          <Select
                            ...
                            onValueChange={(v) =>
                              updateAutomation(idx, {
                                when: {
                                  ...(rule.when || { operator: 'equals' }),
                                  operator: v as AutomationOperator,
                                },
                              })
                            }
                          >
                      ...
                      <div className="flex flex-wrap gap-1">
                        {(rule.add_tag_ids || []).map((id) => {
                          ...
                          return (
                            ...
                            >
                              {t.name}
                              <button onClick={() => toggleAutomationTag(idx, id)}>
                          ...
                          {(leadTags || []).map((t) => {
                            ...
                            return (
                              <button
                                ...
                                className={cn(
                                  'w-full text-left flex items-center gap-2 px-2 py-1 rounded text-xs hover:bg-muted',
                                  checked && 'bg-primary/10',
                                )}
                              >
                                <span
                      ...
                      <div className="grid grid-cols-3 gap-1.5">
                        <Select
                          ...
                          onValueChange={(v) =>
                            updateAutomation(idx, { set_stage_id: v === 'none' ? null : v })
                          }
                        >
                          <SelectTrigger className="h-7 text-xs">
                        ...
                        <Select
                          ...
                          onValueChange={(v) =>
                            updateAutomation(idx, {
                              set_temperature: v === 'none' ? null : (v as LeadTemperature),
                            })
                          }
                        >
                          <SelectTrigger className="h-7 text-xs">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateAutomation(idx, { add_score: parseInt(e.target.value) || 0 })
                          }
                          placeholder="Score"
                          className="h-7 text-xs"
                        />
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(['horizontal', 'vertical'] as const).map((opt) => {
                      const current =
                        ((localBlock.block_settings as any)?.orientation as string) || 'horizontal';
                  ...
                  {!!(localBlock.block_settings as any)?.cta_enabled && (
                    <div className="space-y-2 pl-1">
                      ...
                      <label className="flex items-center gap-2 text-xs">
                        <Switch
                          checked={
                            ((localBlock.block_settings as any)?.cta_target || '_blank') === '_blank'
                          }
                          onCheckedChange={(c) =>
                            updateSettings({ cta_target: c ? '_blank' : '_self' })
                          }
                        />
                        Abrir em nova aba
                      </label>
          ...
          {localBlock.block_type === 'divider' && (
            <>
              <Separator />
              <p className="text-xs text-muted-foreground">
                Divisor visual entre seções. Sem configuração.
              </p>
            </>
          )}
