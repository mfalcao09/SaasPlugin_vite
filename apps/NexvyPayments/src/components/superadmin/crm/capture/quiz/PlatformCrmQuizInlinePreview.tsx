...
/**
 * CRM de PLATAFORMA (super_admin) — preview inline do Quiz no painel direito do FlowTab.
 * DESACOPLADO do tenant. Porte de `admin/capture/quiz/QuizInlinePreview.tsx`.
 *
 * Adaptação: `funnel` é `PlatformCrmCaptureFunnel` (sem organization_id) em vez do `Funnel`
 * de tenant. O iframe aponta para o origin atual (`/q/<slug>`), sem hooks de tenant.
 */
...
export function PlatformCrmQuizInlinePreview({ funnel, refreshKey = 0 }: Props) {
  const [device, setDevice] = useState<'mobile' | 'desktop'>('mobile');
  const [manualReload, setManualReload] = useState(0);
  const iframeRef = useRef<HTMLIFrameElement>(null);

  // Usa o origin atual (editor/preview/published) para garantir que o iframe
  // rode no mesmo build em que o usuário está editando.
