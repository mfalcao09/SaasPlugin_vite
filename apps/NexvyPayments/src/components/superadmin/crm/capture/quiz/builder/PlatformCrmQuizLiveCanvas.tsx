...
import type { PlatformCrmQuizPaletteItem } from '../PlatformCrmQuizCategorizedPalette';
import type { PlatformCrmCaptureFunnel } from '@/components/superadmin/crm/data/usePlatformCrmCaptureFunnels';

/**
 * CRM de PLATAFORMA (super_admin) — canvas central (preview editável ao vivo) do QuizBuilder.
 * DESACOPLADO do tenant. Porte de `admin/capture/quiz/builder/QuizLiveCanvas.tsx`.
 *
 * Adaptação: `funnel` é `PlatformCrmCaptureFunnel` (sem organization_id). A aparência é lida
 * do objeto funnel em memória via `getChannelAppearance(funnel as any, 'quiz')` — a assinatura
 * aceita `any`, então não há acoplamento ao `Funnel` de tenant. Sem hooks de tenant.
 */
...
interface Props {
  ...
  onDropPaletteItem?: (item: PlatformCrmQuizPaletteItem) => void;
}
 ...
 */
export function PlatformCrmQuizLiveCanvas({
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    e.preventDefault();
    try {
      const item = JSON.parse(raw) as PlatformCrmQuizPaletteItem;
