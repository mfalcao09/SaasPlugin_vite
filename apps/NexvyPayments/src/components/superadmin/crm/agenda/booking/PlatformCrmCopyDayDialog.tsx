...
import {
  DAY_NAMES,
  DAY_ABBREVIATIONS,
  type PlatformCrmUserAvailability,
} from '@/components/superadmin/crm/data/usePlatformCrmUserAvailability';

/**
 * Diálogo "copiar horários de um dia para outros" da aba Disponibilidade do
 * CRM de PLATAFORMA (super_admin) — port 1:1 do `CopyDayDialog` do CRM Vendus.
 * Componente puro (sem I/O nem org); só a origem de `DAY_NAMES`/tipo mudou.
 */

interface PlatformCrmCopyDayDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  fromDay: number;
  slots: PlatformCrmUserAvailability[];
  onCopy: (toDays: number[]) => void;
  isLoading?: boolean;
}

export function PlatformCrmCopyDayDialog({
  open,
  onOpenChange,
  fromDay,
  slots,
