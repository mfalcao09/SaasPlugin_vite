...
import {
  usePlatformCrmUserAvailability,
  DAY_NAMES,
  DAY_ABBREVIATIONS,
} from '@/components/superadmin/crm/data/usePlatformCrmUserAvailability';
...
import { PlatformCrmCopyDayDialog } from './PlatformCrmCopyDayDialog';
import { PlatformCrmAddTimeSlotDialog } from './PlatformCrmAddTimeSlotDialog';
import { ScrollArea } from '@/components/ui/scroll-area';

/**
 * DISPONIBILIDADE (grade semanal + exceções por data) do CRM de PLATAFORMA
 * (super_admin) — port 1:1 do `AvailabilityManager` do CRM Vendus. Dados via
 * `usePlatformCrmUserAvailability` (`platform_crm_user_availability` +
 * `platform_crm_availability_overrides`), sem organization_id. Tema claro/rosa
 * via tokens; sem tocar agenda do salão.
 *
 * Horários de funcionamento (business hours) da plataforma são configurados
 * separadamente via `usePlatformCrmBusinessHours` (já portado no core).
 */
...
export function PlatformCrmAvailabilityManager() {
  const {
    availability,
    availabilityByDay,
    overrides,
  ...
  const handleQuickAddSlot = (day: number) => {
    ...
    addTimeSlot.mutate({ day_of_week: day, start_time: startTime, end_time: endTime });
  };

  const handleOpenOverrideDialog = (date: Date | undefined) => {
    if (!date) return;
    setSelectedDate(date);
    const existingOverride = overrides.find((o) => o.date === format(date, 'yyyy-MM-dd'));
    if (existingOverride) {
      setOverrideForm({
        is_available: existingOverride.is_available ?? false,
  ...
  const overrideDates = overrides.reduce((acc, o) => {
    acc[o.date] = o.is_available ?? false;
    return acc;
            ...
            <CardContent className="space-y-3">
              {[1, 2, 3, 4, 5, 6, 0].map((day) => {
                ...
                return (
                  <div
                    key={day}
                    className={cn('rounded-lg border p-3 transition-colors', hasSlots ? 'bg-background' : 'bg-muted/30')}
                  >
                          ...
                          slots.map((slot) => (
                            <div key={slot.id} className="flex items-center gap-2 flex-wrap">
                              <Select
                                ...
                                onValueChange={(v) => updateTimeSlot.mutate({ id: slot.id, start_time: v, end_time: slot.end_time })}
                              >
                              ...
                              <Select
                                value={slot.end_time}
                                onValueChange={(v) => updateTimeSlot.mutate({ id: slot.id, start_time: slot.start_time, end_time: v })}
                              >
                                <SelectTrigger className="w-[90px] h-8 text-xs">
              ...
              <div className="pt-4 border-t flex flex-wrap gap-2">
                <Button variant="outline" size="sm" onClick={() => handleOpenAddSlotDialog(1)}>
                  ...
                  <ScrollArea className="h-[180px]">
                    <div className="space-y-2 pr-4">
                      {overrides.map((override) => (
                        <div key={override.id} className="flex items-center justify-between rounded-lg border p-2 text-sm">
                          <div>
