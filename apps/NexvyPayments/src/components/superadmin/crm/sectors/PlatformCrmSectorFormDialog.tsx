...
interface PlatformCrmSectorFormDialogProps {
  sector: PlatformCrmSector | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function PlatformCrmSectorFormDialog({
  sector,
  open,
  onOpenChange,
}: PlatformCrmSectorFormDialogProps) {
  const upsert = useUpsertPlatformCrmSector();
  ...
  useEffect(() => {
    if (sector) {
      setForm({
        ...
        icon: ((sector.icon as IconName) in SECTOR_ICONS
          ? sector.icon
          : 'Building2') as IconName,
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_120px_auto] gap-3 items-end">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm({ ...form, bot_order: parseInt(e.target.value) || 0 })
                }
              />
            </div>
