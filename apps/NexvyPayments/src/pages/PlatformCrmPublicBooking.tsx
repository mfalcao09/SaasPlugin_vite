// PlatformCrmPublicBooking — página PÚBLICA (anon) do fluxo de booking (Calendly
// de reunião de venda) do módulo super-admin `platform_crm_*`.
//
// Porte 1:1 de PublicBooking.tsx (Vendus/CRM). Passos idênticos:
//   list → calendar → form → [conversational] → confirmation.
// Diferenças (somente as pedidas):
//   (a) dados vêm dos hooks usePlatformCrm* (edges platform-booking-*), nunca de tabela;
//   (b) tema claro/rosa (tokens do app + cor rosa de fallback);
//   (c) zero organization_id/product_id — desacoplado.
//
// Reusa os subcomponentes de UI já presentes no app (idênticos ao Vendus):
//   @/components/booking/BookingThankYou | ConversationalBooking.
...
import {
  usePlatformPublicProfile,
  usePlatformPublicEventTypes,
  usePlatformPublicEventTypeBySlug,
  usePlatformAvailableSlots,
  usePlatformSubmitBooking,
  type PlatformBookingEventType,
} from '@/hooks/usePlatformCrmPublicBooking';
import { BookingThankYou } from '@/components/booking/BookingThankYou';
import { ConversationalBooking, type BookingFormData } from '@/components/booking/ConversationalBooking';
import { format, addDays, startOfDay, isBefore, isAfter } from 'date-fns';
import { ptBR } from 'date-fns/locale';

// Cor de fallback: rosa da marca (o Vendus usava verde #10b981).
const FALLBACK_COLOR = '#e84393';
...
export default function PlatformCrmPublicBooking() {
  ...
  // Queries
  const { data: profile, isLoading: loadingProfile } = usePlatformPublicProfile(userSlug);
  const { data: eventTypes, isLoading: loadingEventTypes } = usePlatformPublicEventTypes(userSlug);
  const { data: eventData, isLoading: loadingEventType } = usePlatformPublicEventTypeBySlug(userSlug, eventSlug);
  const { data: slots, isLoading: loadingSlots } = usePlatformAvailableSlots(
    eventData?.eventType?.id,
    selectedDate ? format(selectedDate, 'yyyy-MM-dd') : undefined,
  );
  const submitBooking = usePlatformSubmitBooking();

  const eventType = eventData?.eventType;
  const hostProfile = eventData?.profile || profile;

  // Reset ao trocar de evento — decide step por booking_experience.
  useEffect(() => {
  ...
  const handleSelectEventType = (et: PlatformBookingEventType) => {
  ...
  const eventColor = eventType?.color || FALLBACK_COLOR;
      ...
      <div className="max-w-4xl mx-auto py-8 px-4">
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div
                            ...
                            style={{ backgroundColor: et.color || FALLBACK_COLOR }}
                          />
              ...
              <div className="flex flex-col md:flex-row">
                ...
                <div className="md:w-64 p-6 border-b md:border-b-0 md:border-r bg-muted/30">
                  ...
                  <Avatar className="h-12 w-12 mb-3">
                    <AvatarImage src={hostProfile?.avatar_url || undefined} />
                    <AvatarFallback>{hostProfile?.full_name?.charAt(0).toUpperCase()}</AvatarFallback>
                  </Avatar>
              ...
              <div className="flex flex-col md:flex-row">
                ...
                <div className="flex-1 p-6">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {/* Perguntas custom — filtra as que duplicam Nome/E-mail/Telefone */}
                    {eventType.questions
                      ?.filter((question) => {
                    ...
                    <Button type="submit" className="w-full" disabled={submitBooking.isPending}>
        ...
        {step === 'confirmation' && eventType && selectedDate && selectedSlot && bookingResult && (
          <BookingThankYou
            ...
            whatHappens={
              Array.isArray(eventType.what_happens)
                ? (eventType.what_happens as { icon: string; title: string; description: string }[])
                : undefined
            }
            nextSteps={
              Array.isArray(eventType.next_steps)
                ? (eventType.next_steps as { icon: string; text: string }[])
                : undefined
            }
            color={eventColor}
