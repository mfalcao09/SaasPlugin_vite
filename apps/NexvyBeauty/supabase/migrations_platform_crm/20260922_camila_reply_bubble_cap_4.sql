-- Sobe o teto de bolhas da resposta de 2 para 4.
-- A função viva não tem a janela comercial do arquivo 20260912; só o limite muda.
-- A constraint da tabela também recusava bubble_count > 2.

ALTER TABLE public.platform_crm_agent_action_ledger
  DROP CONSTRAINT IF EXISTS platform_crm_agent_action_ledger_check;

ALTER TABLE public.platform_crm_agent_action_ledger
  ADD CONSTRAINT platform_crm_agent_action_ledger_check
  CHECK (status = 'denied' OR (bubble_count >= 1 AND bubble_count <= 4));

CREATE OR REPLACE FUNCTION public.pcrm_authorize_and_reserve_agent_action(p_idempotency_key text, p_product_id uuid, p_lead_id uuid, p_conversation_id uuid, p_agent_id uuid, p_instance_id uuid, p_channel text, p_action_type text, p_proactive boolean, p_bubble_count integer, p_content_hash text, p_strategy_version_id uuid DEFAULT NULL::uuid, p_experiment_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  ELSIF p_bubble_count NOT BETWEEN 1 AND 4 THEN
    v_reason := 'bubble_cap';
