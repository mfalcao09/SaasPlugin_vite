...
-- PASSO 2/2 (por-lead) — FLIP DA VIEW: a Base consolidada passa a gatear POR LEAD.
--
...
--    de aprovar-por-lead estar em produção, a Base consolidada continua em 0 (clean
--    slate: todo lead nasce approved_at=NULL) e não há como aprovar leads pela tela.
--
-- Projeto: fzhlbwhdejumkyqosuvq (PG 17.6).
--
-- O QUE MUDA vs. 20260716d (fonte LIVE atual, portão por-EXTRAÇÃO):
--   ÚNICA diferença → o predicado do portão. Sai o filtro por extração
--     `extraction_id IN (SELECT id FROM platform_crm_lead_extractions WHERE approved_at IS NOT NULL)`
--   e entra o filtro POR LEAD, no PRÓPRIO lead:
--     `approved_at IS NOT NULL`
--   nas DUAS CTEs (agg e winner). Consequência: um @handle aparece na base se ≥1
--   das SUAS LINHAS (leads) foi aprovada; o merge/COALESCE/origin_count passam a
--   considerar só linhas de leads aprovados. TODO o resto (dedup, merge, COALESCE,
--   GRANT authenticated, security_invoker=on) é IDÊNTICO ao 20260716d.
--
-- LIMPEZA OPCIONAL (pós-flip, quando a controladora confirmar que a view não
-- referencia mais as colunas da extração): as órfãs de platform_crm_lead_extractions
-- podem ser dropadas com fonte única garantida —
--   ALTER TABLE public.platform_crm_lead_extractions
--     DROP COLUMN IF EXISTS approved_at, DROP COLUMN IF EXISTS approved_by;
-- (NÃO faz parte deste flip; deixado documentado para o passo de limpeza.)
-- ════════════════════════════════════════════════════════════════════════════
...
WITH agg AS (
  ...
  WHERE handle IS NOT NULL AND handle <> ''
    -- PORTÃO (unidade = o LEAD): só linhas de leads APROVADOS entram na base consolidada.
...
winner AS (
  ...
  FROM public.platform_crm_extracted_leads
  WHERE handle IS NOT NULL AND handle <> ''
    -- PORTÃO: idem — o vencedor sai só de leads aprovados.
    AND approved_at IS NOT NULL
  ORDER BY
...
SELECT
  ...
  w.created_at,
  a.origin_count,                          -- em quantas linhas/extrações (aprovadas) aparece
