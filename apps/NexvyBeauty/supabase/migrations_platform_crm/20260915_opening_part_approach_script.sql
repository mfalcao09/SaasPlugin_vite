-- opening_part: bolhas 2–4 do script de abordagem Estágio 1
ALTER TABLE public.platform_crm_agent_action_ledger
  DROP CONSTRAINT IF EXISTS platform_crm_agent_action_ledger_action_type_check;

ALTER TABLE public.platform_crm_agent_action_ledger
  ADD CONSTRAINT platform_crm_agent_action_ledger_action_type_check
  CHECK (action_type IN ('opening', 'opening_part', 'followup', 'resume', 'reply'));
...
BEGIN
  ...
  ELSIF p_action_type NOT IN ('opening', 'opening_part', 'followup', 'resume', 'reply') THEN
  ...
  IF v_reason IS NULL THEN
    ...
    ELSIF p_action_type = 'opening_part' THEN
      ...
      END IF;
      -- sem proactive_daily/interval: mesma sessão de abordagem
