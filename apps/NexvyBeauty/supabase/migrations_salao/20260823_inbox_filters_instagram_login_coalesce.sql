-- Inbox tenant: filtro de conexão Instagram lê Login + legado.
-- Webhook grava metadata.instagram_login_connection_id; RPC antigo só lia instagram_connection_id.
    ...
    WHERE
      ...
      AND (
        ...
        OR (
          ...
          OR (
            NULLIF(COALESCE(NULLIF(c.metadata->>'instagram_login_connection_id', ''), NULLIF(c.metadata->>'instagram_connection_id', '')), '') IS NOT NULL
            AND ('instagram:' || COALESCE(NULLIF(c.metadata->>'instagram_login_connection_id', ''), NULLIF(c.metadata->>'instagram_connection_id', ''))) = ANY(p_connection_keys)
          )
        )
      )
      AND (
