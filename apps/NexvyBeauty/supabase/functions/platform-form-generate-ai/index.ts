// platform-form-generate-ai — GERA formulários de captação com IA para um PRODUTO
// do CRM de PLATAFORMA (super_admin), product-scoped.
//
// Porte 1:1 do org-scoped `form-generate-ai`, DESACOPLADO do tenant. A LÓGICA de
// geração (prompt, contexto, chamada ao AI gateway, parsing robusto do JSON,
// normalização de blocos, shape da resposta) é CÓPIA FIEL do original — só o
// GATE DE ESCOPO muda:
//   * Escopo: product_id → tabelas platform_crm_* (nunca `products` do tenant).
//   * Fonte do produto: platform_crm_products (SEM organization_id). `.single()`
//     preservado → 404 "Produto não encontrado" idêntico ao original.
//   * Cérebro/knowledge: platform_crm_product_knowledge_sources (mesmos filtros
//     status='processed' + is_active). Materiais: platform_crm_agent_training_materials
//     (coluna `extracted_content`, não `content` — mapeamento fiel coluna-a-coluna,
//     igual à twin platform-generate-agent-ai).
//   * Objeções: platform_crm_objections.
//   * Auth: super_admin via authenticatePlatformAgent (JWT do usuário via
//     getClaims + gate super_admin em user_roles, OU service_role + actorUserId
//     no body). RLS não se aplica ao edge (roda com SERVICE_ROLE); o gate é
//     re-aplicado em código.
//   * IA: MESMA chave da plataforma (AI_API_KEY), mesmo provider/prompt do original.
...
serve(async (req) => {
  ...
  try {
    ...
    const bodyParsed = await req.json() as GenerateFormRequest;
    ...
    // Build product context (using REAL columns from platform_crm_products)
...
IMPORTANTE: O array deve conter exatamente ${num_questions} blocos de pergunta + welcome_screen + end_screen (total: ${num_questions + 2} blocos).`;
    ...
    console.error('Error in platform-form-generate-ai:', error);
