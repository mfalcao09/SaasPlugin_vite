// platform-whatsapp-qr-webhook — recebe eventos do Evolution Go p/ o CRM de
...
//   * Canal-por-conversa: wa_qr_instance_id na conversa + product_id
...
function normalizePayload(payload: any): Normalized | null {
  ...
  if (event === "QRCode" || event === "QR" || event === "QRCodeUpdated") {
    ...
    if (!qr) {
      try {
        console.warn("[platform-whatsapp-qr-webhook] QRCode event sem QR extraível — payload:",
...
async function ensureConversation(
  ...
  let conversation: any | null = pickCanonicalWaQrConversation(candidates);

  // Perdedora de merge: NÃO reabrir — seguir merged_into (mesmo fora do batch).
  if (conversation) {
    const mergeTarget = mergedIntoTargetId(conversation);
    if (mergeTarget && mergeTarget !== conversation.id) {
      ...
      if (targetRow) {
        console.log(
          `[platform-whatsapp-qr-webhook] follow merged_into ${conversation.id} → ${mergeTarget}`,
        );
        conversation = targetRow;
      }
    }
  }
  ...
  if (!conversation) {
    ...
    const { data: created, error } = await supabase
      ...
      .single();
    if (error) {
      console.error("[platform-whatsapp-qr-webhook] create conversation failed:", error);
      return null;
    }
 ...
 *  `suppressBrain` → auto-resposta ou apresentar em curso: brain não responde.
...
type ColdOutreachInboundVerdict = { ok: boolean; optOut: boolean; suppressBrain: boolean };
...
// padrão de platform-cold-outreach -> platform-whatsapp-qr-send.
...
async function notifyColdOutreachInbound(
  ...
  try {
    ...
    if (error) {
      console.error(
        `[platform-whatsapp-qr-webhook] cold-outreach on-inbound FAILED conversation_id=${a.conversationId} reason=${error?.message ?? String(error)}`,
      );
      return { ok: false, optOut: false, suppressBrain: false };
    }
    const res = data as
      | { ok?: boolean; intent?: string; affected?: number; handoff?: unknown; error?: string; suppress_brain?: boolean }
      | null;
    if (!res || res.ok !== true) {
      console.error(
        `[platform-whatsapp-qr-webhook] cold-outreach on-inbound REJECTED conversation_id=${a.conversationId} response=${JSON.stringify(res)}`,
      );
      return { ok: false, optOut: false, suppressBrain: false };
    }
    if (res.intent === "opt_out") {
      ...
      console.warn(
        `[platform-whatsapp-qr-webhook] cold-outreach OPT-OUT conversation_id=${a.conversationId} telefone=${a.telefone} queue_rows=${res.affected ?? 0}`,
      );
      return { ok: true, optOut: true, suppressBrain: true };
    }
    if (res.suppress_brain === true || res.intent === "auto_reply") {
      console.log(
        `[platform-whatsapp-qr-webhook] cold-outreach suppress brain conversation_id=${a.conversationId} intent=${res.intent}`,
      );
      return { ok: true, optOut: false, suppressBrain: true };
    }
    console.log(
      `[platform-whatsapp-qr-webhook] cold-outreach on-inbound ok conversation_id=${a.conversationId} intent=${res.intent} affected=${res.affected ?? 0} handoff=${JSON.stringify(res.handoff ?? null)}`,
    );
    return { ok: true, optOut: false, suppressBrain: false };
  } catch (e: any) {
    console.error(
      `[platform-whatsapp-qr-webhook] cold-outreach on-inbound EXCEPTION conversation_id=${a.conversationId} reason=${e?.message ?? String(e)}`,
    );
    return { ok: false, optOut: false, suppressBrain: false };
  }
}
...
async function dispatchSalesBrain(conversationId: string): Promise<void> {
  ...
  if (!base || (!brainSecret && !serviceKey)) {
    console.error(
      `[platform-whatsapp-qr-webhook] brain NÃO despachado conversation_id=${conversationId} — faltam SUPABASE_URL e/ou credencial interna (BRAIN_INTERNAL_SECRET|SUPABASE_SERVICE_ROLE_KEY); a lead ficaria sem resposta`,
    );
    return;
  }
  ...
  const call = fetch(`${base}/functions/v1/platform-sales-brain`, {
    ...
    .then(async (r) => {
      if (!r.ok) {
        console.error(
          `[platform-whatsapp-qr-webhook] brain retornou ${r.status} conversation_id=${conversationId} body=${(await r.text()).slice(0, 200)}`,
        );
      }
    })
    .catch((e) =>
      console.error(
        `[platform-whatsapp-qr-webhook] brain fetch error conversation_id=${conversationId} reason=${e?.message ?? String(e)}`,
      )
    );
...
async function transcribeEvolutionAudio(
  ...
  try {
    ...
    if (!b64) {
      ...
      if (!b64) {
        console.error(
          `[platform-whatsapp-qr-webhook] audio: sem base64 inline E download falhou message_id=${messageId} — segue '[áudio]'`,
        );
        return null;
      }
    }
    ...
    try {
      ...
      console.error(`[platform-whatsapp-qr-webhook] audio: base64 inválido message_id=${messageId}`);
      return null;
    }
    if (bytes.length === 0 || bytes.length > 25 * 1024 * 1024) {
      console.error(`[platform-whatsapp-qr-webhook] audio: tamanho inválido ${bytes.length}`);
      return null;
    }
    ...
    try {
      ...
      if (upErr) {
        console.error("[platform-whatsapp-qr-webhook] audio: upload storage falhou", upErr.message);
      } else {
        const { data: pub } = supabase.storage.from("inbox-media").getPublicUrl(path);
        mediaUrl = pub?.publicUrl ?? null;
      }
    } catch (e) {
      console.error("[platform-whatsapp-qr-webhook] audio: upload exceção", String(e).slice(0, 160));
    }
    ...
    if (!geminiKey) {
      console.error("[platform-whatsapp-qr-webhook] audio: GEMINI_API_KEY ausente — segue sem transcrição");
    } else {
      ...
      if (!trRes.ok || !t || t === "[inaudivel]") {
        console.error(
          "[platform-whatsapp-qr-webhook] audio: gemini falhou",
          trRes.status,
    ...
    console.error("[platform-whatsapp-qr-webhook] audio: exceção", String(e).slice(0, 200));
    return null;
  }
}
  ...
  try {
    ...
    for (const apikey of keys) {
      for (const body of queries) {
        ...
        if (lid) {
          console.log(
            `[platform-whatsapp-qr-webhook] LID lookup ok message_id=${opts.messageId || ""} lid=${lid}`,
          );
          return lid;
        }
      }
    ...
    console.warn(
      `[platform-whatsapp-qr-webhook] LID lookup miss message_id=${opts.messageId || ""} phone=${opts.phoneDigits || ""}`,
    );
    return "";
  } catch (e) {
    console.warn(
      "[platform-whatsapp-qr-webhook] LID lookup exception:",
 ...
 *  opts.skipBrain: checks/sintéticos — grava inbound sem acordar o sales-brain. */
async function handleMessage(
  supabase: any,
  instance: any,
  norm: Extract<Normalized, { kind: "message" }>,
  opts: { skipBrain?: boolean } = {},
): Promise<Response> {
  ...
  if (norm.fromMe) {
    ...
    if (error) {
      if (!String(error.code).includes("23505")) {
        console.error("[platform-whatsapp-qr-webhook] insert device outbound failed:", error);
      }
  ...
  // Path A (PRD-10 / F3): classificar ANTES de qualquer mutação Path A.
  // shadow|enforce: grava decisão canônica no metadata da mensagem.
  // shadow/off: apply_mutation=false e path_a_sends=0 (contenção emergencial
  // em ensureConversation permanece — DNC/remarketing não reabre).
  const pathAMode = getReopenIntentMode();
  if (pathAMode !== "off" && isPathAProtectedClosed(conversation)) {
...
async function findWebhookInstance(
  ...
  for (
    const q of [
      supabase.from("platform_crm_wa_qr_instances").select(selection).eq(
        "instance_id",
        instanceRef,
      ).limit(1),
      supabase.from("platform_crm_wa_qr_instances").select(selection).eq(
...
async function handleAuthorizedWebhook(
  ...
  console.log(
    "[platform-whatsapp-qr-webhook] raw event:",
    rawEvent,
    "instance:",
    rawInstance || "<MISSING>",
  );
  ...
  if (rawEvent === "messages.update" || rawEvent === "MESSAGES_UPDATE") {
    try {
      ...
      console.warn(
        "[platform-whatsapp-qr-webhook] ACK de entrega falhou (non-fatal):",
        String(e).slice(0, 200),
      );
    }
    return new Response(
  ...
  const norm = isZapiWebhookPayload(payload)
    ? normalizeZapiWebhook(payload, String(instance.instance_id || instance.name || ""))
  ...
  if (norm.kind === "connection") {
    ...
    await supabase.from("platform_crm_wa_qr_instances").update(updates)
      .eq("id", instance.id);
...
export function createPlatformEvolutionWebhookReceiver(
  ...
  const evolutionHandler = createPlatformEvolutionWebhookHandler({
    createContext,
    ...
    handleAuthorized: (supabase, req, payload, instance) => {
      ...
      const bodySkip = Boolean(
        payload && typeof payload === "object" &&
          (payload as Record<string, any>)._nexvy?.skip_brain === true,
      );
      const skipBrain = url.searchParams.get("skip_brain") === "1" || bodySkip;
      return handleAuthorizedWebhook(supabase, req, payload, instance, { skipBrain });
    },
    ...
    logAuthFailure: (reason) =>
      console.warn(`[platform-whatsapp-qr-webhook] 401 auth reason=${reason}`),
    logHandlerFailure: () =>
      console.error("[platform-whatsapp-qr-webhook] request failed"),
    corsHeaders,
  });
  ...
  return async (request: Request) => {
    ...
    if (url.searchParams.get("provider") === "zapi") {
      const iid = (url.searchParams.get("iid") || "").trim();
      const tok = (url.searchParams.get("tok") || "").trim();
      const key = (url.searchParams.get("k") || "").trim();
      const expectedKey = String(Deno.env.get("ZAPI_WEBHOOK_KEY") || "").trim();

      try {
        const supabase = createContext();
        ...
        if (iid && tok) {
          const { data } = await supabase
            ...
            .eq("instance_id", iid)
            .maybeSingle();
          const expected = String(data?.instance_token ?? "").trim();
          let mismatch = expected.length !== tok.length || !expected;
          if (!mismatch) {
            let diff = 0;
            for (let i = 0; i < expected.length; i++) {
              diff |= expected.charCodeAt(i) ^ tok.charCodeAt(i);
            }
            mismatch = diff !== 0;
          }
          if (!mismatch) instance = data;
        }

        // 2) Auth curta: ?k=WEBHOOK_KEY (+ resolve por connectedPhone ou única zapi)
        if (!instance && expectedKey && key && key === expectedKey) {
          const connectedPhone = String(
            payload.connectedPhone ?? payload.phone ?? "",
          ).replace(/\D/g, "");
          if (connectedPhone) {
            const { data } = await supabase
              .from("platform_crm_wa_qr_instances")
              ...
              .limit(1)
              .maybeSingle();
            if (data && (data.metadata as any)?.provider === "zapi") instance = data;
            // também aceita se metadata.provider ausente mas instance_id hex longo
          ...
          if (!instance) {
            const { data: rows } = await supabase
              ...
              .select("*")
              .eq("status", "connected")
              .contains("metadata", { provider: "zapi" })
              .limit(2);
            if (rows && rows.length === 1) instance = rows[0];
          }
          if (!instance) {
            // fallback: única instância connected com metadata provider zapi via filter js
            const { data: all } = await supabase
              .from("platform_crm_wa_qr_instances")
              .select("*")
              .eq("status", "connected");
            const zapiRows = (all ?? []).filter((r: any) =>
              String(r?.metadata?.provider || "").toLowerCase() === "zapi"
            );
            if (zapiRows.length === 1) instance = zapiRows[0];
          }
        }

        if (!instance) {
          console.warn("[platform-whatsapp-qr-webhook] 401 auth reason=zapi_unauthorized");
        ...
        const skipBrain = url.searchParams.get("skip_brain") === "1" || bodySkip;
        return await handleAuthorizedWebhook(supabase, request, payload, instance, {
          skipBrain,
        });
      } catch (e) {
        console.error("[platform-whatsapp-qr-webhook] zapi handler failed", String(e).slice(0, 200));
    ...
    return evolutionHandler(request);
  };
}
