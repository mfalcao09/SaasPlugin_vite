...
const TRANSPARENT_PROMPT =
  ...
  `responda com honestidade e não negue — diga que é a Camila, assistente automatizada da Nexvy, e siga ajudando.\n` +
...
Deno.test("required fragments: Nexvy/Camila, Instagram público, honestidade IA, PARE/opt-out, preço do contexto", () => {
  for (
    const frag of [
      "Camila",
      "Nexvy",
    ...
    assertEquals(
      ...
      true,
      `fragmento obrigatório ausente da exportação: ${frag}`,
    );
  }
});
...
Deno.test("assertCamilaIdentityCompliant: rejeita frase proibida", () => {
  ...
  assertThrows(
    () =>
      assertCamilaIdentityCompliant(
        TRANSPARENT_PROMPT.replace("não negue", "NUNCA assume ser IA"),
      ),
    Error,
    "NUNCA assume ser IA",
  );
