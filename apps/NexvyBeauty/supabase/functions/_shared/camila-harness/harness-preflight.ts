...
export async function outboundAlreadySent(
  ...
  if (!phones.length || !input.productId) {
    return { sent: false, error: "missing_phone_or_product" };
  }
  const base = () =>
    sb.from("platform_crm_messages")
      .select("id, platform_crm_conversations!inner(product_id, visitor_phone)")
  ...
  if (byKey.error) {
    return { sent: false, error: byKey.error.message ?? "lookup_failed" };
  }
  if ((byKey.data?.length ?? 0) > 0) return { sent: true, error: null };

  const byText = await base().eq("content", input.text);
  if (byText.error) {
    return { sent: false, error: byText.error.message ?? "lookup_failed" };
  }
  return { sent: (byText.data?.length ?? 0) > 0, error: null };
}
