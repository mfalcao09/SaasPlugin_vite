// platform-quiz-generate-ai — GERA quizzes de qualificação com IA para um PRODUTO
// do CRM de PLATAFORMA (super_admin), product-scoped.
//
// Porte 1:1 do org-scoped `quiz-generate-ai`, DESACOPLADO do tenant. A LÓGICA de
// geração (prompt, contexto, chamada ao AI gateway, parsing do JSON, shape da
// resposta) é CÓPIA FIEL do original — só o GATE DE ESCOPO muda:
//   * Escopo: product_id → tabelas platform_crm_* (nunca `products` do tenant).
//   * Fonte do produto: platform_crm_products (SEM organization_id). `.maybeSingle()`
//     preservado — product_id é OPCIONAL no quiz; ausência → gera sem contexto de
//     produto (mesmo comportamento do original, agora com a tabela CERTA quando o
//     id vem do CRM de plataforma).
//   * Cérebro/knowledge: platform_crm_product_knowledge_sources (mesmos filtros
//     status='processed' + is_active).
...
serve(async (req) => {
  ...
  try {
    ...
    // Contexto opcional do produto + cérebro (product-scoped — SEM organization_id).
    let productContext = '';
    let knowledgeContext = '';
    if (product_id) {
      const { data: product } = await supabase.from('platform_crm_products').select('*').eq('id', product_id).maybeSingle();
      if (product) {
        productContext = `
...
Retorne APENAS o JSON.`;
    ...
    console.error('platform-quiz-generate-ai error:', error);
