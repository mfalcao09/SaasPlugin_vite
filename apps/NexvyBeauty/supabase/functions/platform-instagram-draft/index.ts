// platform-instagram-draft — cria conexao Instagram Direct em "rascunho" no CRM
// de PLATAFORMA. Porte 1:1 do `instagram-draft`, DESACOPLADO do tenant:
//   * Tabela: platform_crm_instagram_connections (sem organization_id).
//   * Auth: super_admin via authenticatePlatformAgent.
//   * Webhook URL -> platform-instagram-webhook/{id} (fase de inbox; URL exibida no wizard).
...
Deno.serve(async (req: Request) => {
  ...
  if (connection_id) {
    ...
    if (existing) {
      return json({
        ...
        webhook_url: `${supabaseUrl}/functions/v1/platform-instagram-webhook/${existing.id}`,
  ...
  return json({
    ...
    webhook_url: `${supabaseUrl}/functions/v1/platform-instagram-webhook/${row.id}`,
