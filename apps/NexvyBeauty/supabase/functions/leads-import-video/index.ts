...
//
// Fluxo (path PRIMÁRIO = FRAMES): o FRONT extrai quadros (frames JPEG) de uma gravação
// de tela rolando o Instagram e envia os frames base64 aqui. Esta edge manda os frames ao
// Gemini (visão, via gateway do app — AI_GATEWAY_URL/AI_API_KEY) em LOTES paralelos pedindo
// TODOS os @handles visíveis, funde+deduplica GLOBALMENTE por handle contra a base do
// produto, e dispara o profile-scraper do Apify (usernames[]) — o MESMO enriquecimento do
// colar-handles. O webhook dedicado (leads-import-video-webhook) baixa o dataset e distribui
// os leads nas buscas do dia. (A CLASSIFICAÇÃO das buscas — c/ wpp vs s/ wpp — está em
// redesenho por sessão paralela; NÃO é decidida/finalizada aqui neste FIX de frames.)
//
// Por que frames é o PRIMÁRIO? O front NUNCA sobe o arquivo grande — extrai quadros locais e
// manda JPEGs pequenos —, então o path de frames "engole" QUALQUER TAMANHO de vídeo (a maioria
// passa de 500MB). O gateway (OpenRouter) NÃO aceita vídeo cru (400 em image_url/video_url/
// file; input_video é ignorado e o modelo alucina) — só imagem (visão), já provada.
//
// Path NATIVO (Files API, GEMINI_API_KEY) = OPÇÃO só p/ clipe PEQUENO (<45MB): sobe o vídeo
// inteiro e o modelo o assiste (sem gap de amostragem). Acima de 45MB (memória/upload) cai p/
// frames. Ver spike documentado no PR.
//
...
// FRAMES é o path PRIMÁRIO (vídeo grande nunca sobe). O teto NÃO é mais a coberturazinha
// de 60/80 quadros — é uma proteção de tempo-de-execução da edge + tamanho do POST. Com
// amostragem densa (2–4 fps) 300 quadros cobrem minutos de scroll; vídeos ainda mais longos
// se dividem (UX de overflow). Os lotes rodam em paralelo (GEMINI_CONCURRENCY) p/ caber no
// budget de tempo. Trade-off honesto: p/ +500MB só frames é viável (nativo não), e a densidade
// mitiga o gap de amostragem — mas o nº de quadros por chamada é limitado por edge/POST.
const MAX_FRAMES = 300;       // teto de frames por chamada (edge time + tamanho do POST).
const GEMINI_BATCH = 8;       // imagens por chamada ao Gemini (path frames).
const GEMINI_CONCURRENCY = 5; // chamadas Gemini em paralelo (cabe o volume no tempo da edge).
const DEFAULT_MODEL = 'google/gemini-2.5-flash';
const VIDEO_BUCKET = 'prospeccao-video';
const NATIVE_MAX_BYTES = 45 * 1024 * 1024; // path nativo SÓ p/ clipe pequeno (<45MB); acima → frames
...
Deno.serve(async (req: Request) => {
  ...
  if (videoPath) {
    ...
    try {
      // Lotes de GEMINI_BATCH imagens, GEMINI_CONCURRENCY em paralelo, merge+dedup por handle.
      const r = await extractHandlesFromFrames(
        frames,
        GEMINI_BATCH,
        (batch) => extractHandlesBatch(batch, model, apiKey),
        GEMINI_CONCURRENCY,
      );
      geminiCalls = r.geminiCalls;
      pushHandles(r.handles);
    } catch (e) {
      const msg = String((e as Error).message ?? e).slice(0, 300);
  ...
  // 1 busca por (fonte, data) — contrato 2026-07-19. A aba com/link/sem whatsapp é
  // DERIVADA em runtime via classifyWhatsapp no read; NÃO há split físico c/s aqui.
  ...
  // 3) find-or-create da busca ÚNICA do dia (dona do run — guarda os run_ids).
  let bucketId: string;
  try {
    bucketId = await findOrCreateBucket(sb, productId, label, {
      apify_actor_id: PROFILE_ACTOR_ID,
      requested_by: user?.id ?? null,
      params: { via: 'leads-import-video', day, run_ids: [] },
    });
  } catch (e) {
  ...
  const webhookUrl =
    ...
    `/leads-import-video-webhook?extraction_id=${bucketId}`;

  // 4) Dispara o profile-scraper (usernames[]) async.
  try {
    const run = await startProfileScraperRun(netNew, webhookUrl, apifyToken);
    // Acumula o run id na busca (suporta append de várias importações no mesmo dia).
    const { data: cur } = await sb
      .from('platform_crm_lead_extractions').select('params').eq('id', bucketId).maybeSingle();
    const prevRunIds: string[] = Array.isArray(cur?.params?.run_ids) ? cur.params.run_ids : [];
    const params = {
      ...(cur?.params ?? {}),
      via: 'leads-import-video', day,
    ...
    await sb.from('platform_crm_lead_extractions')
      .update({ status: 'running', apify_run_id: run.runId, params }).eq('id', bucketId);
    ...
    return json({
      ok: true, day, extraction_id: bucketId, run_id: run.runId,
    ...
    return json({ error: msg, extraction_id: bucketId }, 200);
  }
});
