...
function triageSummary(values: string[]): string {
...
Deno.serve(async (req: Request) => {
  ...
  if (action === "cancel") {
    ...
    if (!validUuid(operationId))
      return json({ error: "operation_id invalido" }, 400);
    ...
    if (!data)
      return json({ error: "operação não encontrada ou já encerrada" }, 409);
  ...
  if (!OP_TYPES.has(operationType))
    return json({ error: "operation_type invalido" }, 400);

  const leadId = body?.lead_id == null ? null : String(body.lead_id).trim();
  const extractedLeadId =
    body?.extracted_lead_id == null
      ? null
      : String(body.extracted_lead_id).trim();
  if (!validUuid(leadId) && !validUuid(extractedLeadId)) {
    return json({ error: "informe lead_id ou extracted_lead_id" }, 400);
  }
  if (leadId && !validUuid(leadId))
    return json({ error: "lead_id invalido" }, 400);
  if (extractedLeadId && !validUuid(extractedLeadId))
    return json({ error: "extracted_lead_id invalido" }, 400);

  const idempotencyKey = String(body?.idempotency_key ?? "")
    .trim()
    .slice(0, 180);
  if (!idempotencyKey)
    return json({ error: "idempotency_key obrigatoria" }, 400);
  ...
  const { data: existing, error: existingError } = await sb
    ...
    .maybeSingle();
  if (existingError)
    return json({ error: "falha ao consultar idempotência" }, 500);
  if (existing)
    return json({ ok: true, idempotent: true, operation: existing });

  const { data: operation, error: insertError } = await sb
    .from("platform_crm_lead_operations")
    .insert({
  ...
  if (insertError) {
    if (insertError.code === "23505") {
      const { data: raced } = await sb
        ...
        .maybeSingle();
      return raced
        ? json({ ok: true, idempotent: true, operation: raced })
        : json({ error: "operação duplicada" }, 409);
    }
  ...
  if (operationType === "preselection") {
    if (!leadId) {
      await sb
        ...
        .update({
          status: "failed",
          finished_at: new Date().toISOString(),
          error: "preselection_requires_lead_id",
        })
        .eq("id", operation.id);
    ...
    if (state.derived_stage !== "db" && state.derived_stage !== "preselected") {
      await sb
        ...
        .update({
          status: "failed",
          finished_at: new Date().toISOString(),
          error: `invalid_vertical_stage:${state.derived_stage}`,
        })
        .eq("id", operation.id);
      return json(
        {
          error: "lead não está elegível para pré-seleção",
          stage: state.derived_stage,
        },
        409,
      );
    }
    ...
    if (profileError) {
      await sb
        ...
        .update({
          status: "failed",
          finished_at: new Date().toISOString(),
          error: "triage_read_failed",
        })
        .eq("id", operation.id);
      return json({ error: "falha ao validar categoria do lead" }, 500);
    }
    const category = triageSummary(
      (profiles ?? []).map((profile: any) =>
        String(profile.triagem ?? "nao_classificado"),
      ),
    );
    if (!CAMPAIGN_TRIAGES.has(category)) {
      await sb
        .from("platform_crm_lead_operations")
        .update({
          status: "failed",
          finished_at: new Date().toISOString(),
          error: `needs_reclassification:${category}`,
        })
        .eq("id", operation.id);
      return json(
        {
          error: "lead precisa ser reclassificado antes da pré-seleção",
          triagem: category,
    ...
    if (state.derived_stage === "db") {
      const { data: cas, error: casError } = await sb.rpc(
      ...
      if (casError || !cas?.ok) {
        await sb
          ...
          .update({
            status: "failed",
            finished_at: new Date().toISOString(),
            error: casError?.message ?? "state_cas_conflict",
          })
          .eq("id", operation.id);
    ...
    const { data: done, error: doneError } = await sb
      ...
      .update({
        ...
        finished_at: new Date().toISOString(),
        result: { derived_stage: "preselected" },
      })
      .eq("id", operation.id)
      .select("*")
      .single();
    if (doneError)
      return json(
        { error: "pré-seleção aplicada, mas operação não foi fechada" },
        500,
      );
  ...
  if (operationType === "enrichment") {
    ...
    if (!internalResponse.ok || internalData?.error) {
      await sb
        ...
        .update({
          ...
          error: String(
            internalData?.error ??
              `enrichment_start_${internalResponse.status}`,
          ).slice(0, 500),
        })
        .eq("id", operation.id);
      return json(
        {
          error: "falha ao iniciar enriquecimento",
          detail: internalData?.error ?? null,
        },
        502,
      );
    }
    ...
    const { data: running, error: runningError } = await sb
      ...
      .update({
        ...
        result: {
          ...
          extraction_id: internalData.extraction_id ?? null,
          run_id: internalData.run_id ?? null,
        },
      })
      .eq("id", operation.id)
      .select("*")
    ...
    if (runningError)
      return json(
        { error: "enriquecimento iniciado, mas operação não foi atualizada" },
        500,
      );
