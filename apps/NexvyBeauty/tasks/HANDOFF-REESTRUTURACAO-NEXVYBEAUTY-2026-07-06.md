...
|---|---|---|---|
| **E1 · Pivot D3 multiproduto** | gestao.* deixa de ser CRM de 1 produto → **CRM DO GRUPO** p/ ~10 SaaS via dimensão `product_id`/`platform_crm_products` (não 10 ports). + migração de domínio → `gestao.nexvy.tech`. Sequência travada: D3→domínio→LOTE L1-L13→D1(b)…D10(a)→Utmify por último. | 🟡 **Fase 1a EXECUTADA em prod** (CORRIGIDO 07-06 — não era "não executado"): `platform_crm_products` existe, `product_id` em 23 tabelas (555×), hub de 14 abas, comissão product-aware, "Negócios"→catálogo. **Faltam F1-F6:** `ProductContext` global (G1), módulo por host (G2), reconciliar migrations↔banco (G3), 8 tabelas-fantasma dos stubs (G4), seed dos ~9 produtos (G5). **Plano: [`tasks/PLANO-E1-D3-MULTIPRODUTO-2026-07-06.md`](PLANO-E1-D3-MULTIPRODUTO-2026-07-06.md).** | memória `project_crm_grupo_multiproduto_decisoes_2026-07-02` |
...
| **E6 · Redesign duplo** | Beauty Rosé (app) + Nexvy Lux (gestao). Lux SUPERSEDE o azul `#0A52D1` (não restaurar). | 🟢 Rosé + Lux L1–L3 no ar. **L4 = cirúrgico, FEITO** (branch `feat/beauty-lux-l4` `87cc744`, batchado): gestao JÁ é token-Lux; só 5 acentos de marca (rosa do app antigo no ChatTab/CatalogSync) → token. **NÃO existe "restyle de 47 telas"** — premissa antiga CORRIGIDA; cor semântica (Instagram/status/paletas/negativo) preservada de propósito. | [HANDOFF-REDESIGN-LUX-ROSE](HANDOFF-REDESIGN-LUX-ROSE-2026-07-06.md) |
...
| ~~E6 · Onda L4 do Lux~~ **FEITO cirúrgico** (`87cc744`, batchado) | código | Premissa "47 telas / 16 arquivos p/ limpar" era **FALSA**: gestao já usa tokens de tema (auto-Lux). Só 5 acentos de marca-antiga (rosa) trocados p/ token no ChatTab/CatalogSync. Rosa/azul/rosé remanescentes = **semânticos** (Instagram/status/paletas/negativo), preservados de propósito (diretiva "cuidado com a diferença"). |
