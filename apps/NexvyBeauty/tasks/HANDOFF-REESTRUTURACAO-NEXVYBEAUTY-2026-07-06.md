...
| **5 worktrees sem o fix supabase-js** (`e1f5`/`e1f2`/`e1db`/`beauty`/`nexvypayments-bootstrap`) | sincronização de branch | `_shared/` dessas worktrees ainda tem `esm.sh` + bug `ReturnType<typeof createClient>` (TS2345). Já corrigido e **mergeado em `main`** (`df58f9d`); cada frente herda no próximo merge/rebase — não é bloqueante, mas rodar `deno check --node-modules-dir=none` nessas worktrees ANTES do fix vai mostrar erros já resolvidos no tronco. Detalhe técnico: memória `reference_supabase_js_returntype_createclient_ts2345_2026-07-06`. **Escopo final (commit `4079964`, em `main`):** além do `_shared`+107 esm.sh, 3 functions adjacentes já em npm:@2 (`platform-meta-whatsapp-webhook`, `process-email-queue`, `campaign-start`) tinham o mesmo pattern → zeradas; **diretório inteiro (165 index.ts) passa `deno check` (0 erros)**. |
| **Bug latente `cakto-recovery-trigger`** — filtro de cenário por produto INERTE | código + schema DB | `matchScenario` compara `order.product_cakto_id`, mas o SELECT (~L92) **não busca** essa coluna → sempre `undefined` → cenário com filtro de produto **NUNCA casa**. Revelado pelo type-fix (já em `main`): campo tipado opcional + comentado; **falta decidir** — a coluna existe em `cakto_orders`? adicionar ao SELECT ou trocar p/ `product_id`? Chip `task_ae669d71`. ⚠️ é `cakto_*` → coordenar com a sessão de afiliados em voo (§4.2) antes de mexer no SELECT. |
