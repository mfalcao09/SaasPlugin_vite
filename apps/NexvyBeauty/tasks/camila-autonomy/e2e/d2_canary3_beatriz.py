#!/usr/bin/env python3
"""D2 canary #2 — Beatriz. Uses supabase CLI + vault tick (no VPS JWT).
...
CANARY = {
    "handle": "beatrizsouza_af",
    "extracted_lead_id": "eaf32fc4-eb78-4bab-8a0c-2a1b3dce5399",
    "greeting": "Beatriz",
    "telefone": "5566992054841",
}
...
INSERT INTO platform_crm_cold_campaigns (
  ...
  'd2-canary-beatriz-{stamp}', 'active', false, '{now_iso}',
...
SET status='paused', dry_run=true, activated_at=null, paused_reason='d2_canary3_opening_fail', updated_at=now()
WHERE id='{camp_id}';
...
SET status='paused', dry_run=true, activated_at=null,
    paused_reason='d2_canary3_done', updated_at=now()
WHERE id='{camp_id}';
UPDATE platform_crm_extracted_leads
...
def _write(report: dict, stamp: str) -> Path:
    ...
    path = EVIDENCE / f"d2-canary-beatriz-{stamp}.json"
    path.write_text(json.dumps(report, indent=2, ensure_ascii=False) + "\n")
    (EVIDENCE / "d2-canary-beatriz-latest.json").write_text(path.read_text())
    return path
