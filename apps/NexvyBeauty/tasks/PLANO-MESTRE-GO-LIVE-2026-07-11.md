...
| **P2.A-2 — edges twin (pipelines)** | ✅ **DONE+DEPLOY+PROVADO** | PR #23 `8224db8` (WF `ww9uhyszl`): 9 edges portadas 1:1 + 5 stubs religados · **backend PROVADO: 9/9 `v1·verify_jwt=True·ACTIVE`** (list_edge_functions) · **frontend PROVADO: bundle `index-ByygTONo.js` (era DnoQkIgH) em gestao+app** via gate anti-fantasma + curl externo · gate anti-`organization_id` limpo, tsc delta-0, build verde, 9/9 `authenticatePlatformAgent` · secrets AI_API_KEY/AI_GATEWAY_URL ✅ · ⏳ **1 HITL menor**: `FIRECRAWL_API_KEY` p/ ligar `catalog-sync-website` (edge dormente, sem caller — não bloqueia) |
