...
import {
  ...
  type WeekSchedule,
  type DayKey,
} from '../data/usePlatformCrmBusinessHours';

/**
 * HORÁRIOS de funcionamento do CRM de PLATAFORMA (super_admin) — desacoplados
 * do tenant. Port 1:1 do `BusinessHoursManager` do CRM Vendus. Usa
 * EXCLUSIVAMENTE `usePlatformCrmBusinessHours` (tabelas `platform_crm_business_*`)
 * + componentes @/components/ui. Sem organization/product, sem cockpit do salão.
 *
 * É puro CRUD (agenda semanal + mensagem fora do horário + feriados). Não há
 * Edge Function / canal externo / OAuth no fluxo — nada vira stub.
 */
...
export function PlatformCrmBusinessHoursManager() {
  const { data: bh } = usePlatformCrmBusinessHours();
  const { data: holidays } = usePlatformCrmBusinessHolidays();
  ...
  const isOpenNow = isWithinPlatformCrmBusinessHoursLocal(
...
export default PlatformCrmBusinessHoursManager;
