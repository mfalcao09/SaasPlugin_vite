...
/**
 * Renderizador de mídia (áudio/imagem/vídeo/documento) da inbox do CRM de
 * PLATAFORMA. PORTE de `seller/inbox/MediaAttachment.tsx` (CRM Vendus) — trocas:
 * tema já em tokens (`primary`/`foreground`); desacoplamento: independe do canal
 * (recebe um `PlatformCrmMediaPayload` com URL pronta — a origem da URL/entrega é
 * do host, ver ponto A1-media no ChatArea). UI 1:1 (player WhatsApp-like, lightbox,
 * card de documento).
 */

export type PlatformCrmMediaKind = 'audio' | 'image' | 'video' | 'document' | 'sticker';

export interface PlatformCrmMediaPayload {
  kind: PlatformCrmMediaKind;
  url: string;
  mime?: string | null;
...
interface PlatformCrmMediaAttachmentProps {
  media: PlatformCrmMediaPayload;
  /** true = agente (lado direito, primary). false = visitante (muted). */
  isOwn: boolean;
}
...
export const PlatformCrmMediaAttachment = forwardRef<HTMLDivElement, PlatformCrmMediaAttachmentProps>(
  function PlatformCrmMediaAttachment({ media, isOwn }, ref) {
...
function AudioPlayer({ media, isOwn }: { media: PlatformCrmMediaPayload; isOwn: boolean }) {
...
function ImageViewer({ media }: { media: PlatformCrmMediaPayload }) {
...
function VideoPlayer({ media }: { media: PlatformCrmMediaPayload }) {
  return (
...
function DocumentCard({ media, isOwn }: { media: PlatformCrmMediaPayload; isOwn: boolean }) {
