...
/**
 * Composer de enquete do chat — porte fiel A1.2 de
 * `seller/inbox/PollComposerDialog.tsx` (Vendus v5 original).
 * Sem I/O próprio: devolve `{question, options[]}` para o ChatInput enviar
 * como mensagem formatada.
 */
interface PlatformCrmPollComposerDialogProps {
  open: boolean;
  onOpenChange: (v: boolean) => void;
  onConfirm: (data: { question: string; options: string[] }) => void;
}

export function PlatformCrmPollComposerDialog({
  open,
  onOpenChange,
  onConfirm,
}: PlatformCrmPollComposerDialogProps) {
