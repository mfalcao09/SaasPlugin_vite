// PORTE 1:1 de `.vendus-src-reference/src/components/admin/agents/AgentOrchestratorRoutingTab.tsx`
// D3 P1/F1d — subsistema de AGENTES IA por produto. Twin: tipos de `./types`.
// Rewire tenant->plataforma: matriz de roteamento sobre platform_crm_products +
// platform_crm_product_agents (sem organization_id). Escopo global = product_id null.
...
import { AGENT_TYPE_LABELS, AgentType, ProductAgent } from './types';
...
export function AgentOrchestratorRoutingTab({ currentAgentId, formData, onChange }: Props) {
  const { data, isLoading } = useQuery({
    queryKey: ['platform-crm', 'orchestrator-routing-matrix'],
  ...
  // Todos os agentes ativos, exceto o proprio e outros orquestradores
  ...
  const agentsByProduct = useMemo(() => {
    ...
    // Ordenar por nome do produto, considerando so produtos visiveis
    return products
  ...
  // tool_configs.routing_products / routing_agents - array de IDs incluidos no roteamento.
  // Quando o array esta ausente (undefined), tratamos como "todos selecionados" por padrao.
  ...
  // Helpers para selecao em grupos de agentes
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="text-sm text-muted-foreground">
            ...
            <p>
              ...
              desmarcados serao ignorados na hora de transferir a conversa.
            </p>
          </div>
        </div>
      </div>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between gap-2">
            <CardTitle className="text-sm flex items-center gap-2">
              ...
              Produtos disponiveis ({selectedProductCount}/{products.length})
            </CardTitle>
            {products.length > 0 && (
              <div className="flex gap-1">
                <Button type="button" size="sm" variant="ghost" className="h-7 px-2 text-xs" onClick={selectAllProducts}>
                  Todos
                </Button>
                <Button type="button" size="sm" variant="ghost" className="h-7 px-2 text-xs" onClick={clearProducts}>
                  Nenhum
                </Button>
              </div>
            )}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-2">
                        ...
                        {ok ? (
                          ...
                          <Badge variant="outline" className="gap-1 text-xs border-warning text-warning shrink-0">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {productAgents.length === 0 ? (
                          <span className="text-xs text-muted-foreground italic">
                            Sem especialistas - Orquestrador atendera direto.
                          </span>
                        ) : (
                          productAgents.map((a) => (
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between gap-2">
            ...
            {globalAgents.length > 0 && (
              <div className="flex gap-1">
                <Button type="button" size="sm" variant="ghost" className="h-7 px-2 text-xs" onClick={selectAllGlobalAgents}>
                  Todos
                </Button>
                <Button type="button" size="sm" variant="ghost" className="h-7 px-2 text-xs" onClick={clearGlobalAgents}>
                  Nenhum
                </Button>
              </div>
            )}
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between gap-2">
            ...
            {productAgentsCount > 0 && (
              <div className="flex gap-1">
                <Button type="button" size="sm" variant="ghost" className="h-7 px-2 text-xs" onClick={selectAllProductAgents}>
                  Todos
                </Button>
                <Button type="button" size="sm" variant="ghost" className="h-7 px-2 text-xs" onClick={clearProductAgents}>
                  Nenhum
                </Button>
              </div>
            )}
                ...
                return (
                  <div key={product.id} className="space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex gap-1">
                        <Button type="button" size="sm" variant="ghost" className="h-6 px-2 text-[11px]" onClick={() => setAgentsSelection(groupIds, true)}>
                          Todos
                        </Button>
                        <Button type="button" size="sm" variant="ghost" className="h-6 px-2 text-[11px]" onClick={() => setAgentsSelection(groupIds, false)}>
                          Nenhum
                        </Button>
                      </div>
                    </div>
