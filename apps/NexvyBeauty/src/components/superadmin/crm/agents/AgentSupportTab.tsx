// PORTE 1:1 de `.vendus-src-reference/src/components/admin/agents/AgentSupportTab.tsx`
...
import { ProductAgent, SupportLink, SupportQuickAnswer } from './types';
...
export function AgentSupportTab({ formData, onChange, agentId }: Props) {
  ...
  const updateToolConfigs = (patch: Record<string, unknown>) => {
    onChange({
      tool_configs: {
        ...((formData.tool_configs as Record<string, unknown>) || {}),
        ...patch,
      } as ProductAgent['tool_configs'],
    });
  };
