// Chat de teste do AgentEditor (subsistema de AGENTES por PRODUTO).
// Porte do `admin/agents/AgentTestChat.tsx`, REBASADO na plataforma:
//   • edge  webchat-bot            → platform-webchat-bot (que existe)
//   • id    agent_id (agent_configs) → product_agent_id (platform_crm_product_agents)
// O caminho `agent_id` do edge permanece intacto; aqui usamos SEMPRE
// product_agent_id (o id do agente do AgentEditor). is_test=true → o edge
// responde sem persistir. Zero organization_id/tenant.
...
interface Props {
  /** id do agente em `platform_crm_product_agents` (o agente do AgentEditor). */
  agentId: string;
  agentName: string;
  productId: string | null;
  /** Orquestrador/admin → badge visual; o motor de teste é o mesmo LLM reply. */
  agentType?: string;
}
  ...
  const handleSend = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('platform-webchat-bot', {
        body: {
          // conversa efêmera de teste — o edge com is_test NÃO persiste.
          conversation_id: `agent-test-${agentId}-${sessionId}`,
          message: text,
          // ADITIVO: monta a persona a partir de platform_crm_product_agents.
          product_agent_id: agentId,
          is_test: true,
          // agent_config explícito e mínimo: mantém o teste focado NESTE agente
          // (sem o edge puxar o agent_config do widget ativo, que injetaria
          // instruções/FAQ não relacionadas). is_test não persiste.
          agent_config: {
      ...
      const botContent =
        data?.message?.content || data?.response || data?.reply || 'Sem resposta do agente.';
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-1.5">
              ...
              {isOrchestratorTest && (
                <Badge
                  variant="outline"
                  className="h-4 px-1.5 text-[9px] border-amber-500/40 text-amber-700 bg-amber-50"
                >
                  Orquestrador
                </Badge>
              )}
