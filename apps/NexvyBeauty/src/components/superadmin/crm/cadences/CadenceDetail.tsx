...
import { usePlatformCrmCadenceSteps } from '../data/usePlatformCrmCadences';
...
export function CadenceDetail({ cadenceId, onBack, onEdit }: Props) {
  ...
  const { steps } = usePlatformCrmCadenceSteps(cadenceId);

  useEffect(() => {
    supabase.from('platform_crm_cadences').select('*').eq('id', cadenceId).maybeSingle()
                  ...
                  <div className="flex-1 border rounded-lg p-3 space-y-1">
                    ...
                    {s.context_inline && (
                      <div className="text-xs bg-muted rounded p-2 mt-1 flex items-start gap-2">
                        ...
                        <span className="line-clamp-3">{s.context_inline}</span>
                      </div>
                    )}
                  </div>
                </div>
