...
import {
  usePlatformCrmCampaignContexts,
  type CampaignContext,
} from '../data/usePlatformCrmCampaignContexts';

/**
 * Biblioteca de Contextos (super_admin) — CRUD 1:1 do `ContextLibrary` de tenant,
 * tocando `platform_crm_campaign_contexts`. Sem organization_id no payload:
 * a RLS super_admin-only isola os dados.
 */
export function ContextLibrary() {
  const { contexts, refresh, loading } = usePlatformCrmCampaignContexts();
  ...
  const save = async () => {
    ...
    const { error } = editing.id
      ? await supabase.from('platform_crm_campaign_contexts').update(payload).eq('id', editing.id)
      : await supabase.from('platform_crm_campaign_contexts').insert(payload);
  ...
  const remove = async (id: string) => {
    ...
    const { error } = await supabase.from('platform_crm_campaign_contexts').delete().eq('id', id);
    if (error) { toast.error(error.message); return; }
