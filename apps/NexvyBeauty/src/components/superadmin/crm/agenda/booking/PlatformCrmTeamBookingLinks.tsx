...
import {
  Search, Copy, ExternalLink, MessageCircle, QrCode, Loader2,
  Link2, Users, Download, Sparkles,
} from 'lucide-react';
...
import {
  usePlatformCrmSellerSlugs,
  generatePlatformCrmBookingSlug,
} from '@/components/superadmin/crm/data/usePlatformCrmBookingSlug';

/**
 * LINKS DA EQUIPE (agendamento por vendedor) do CRM de PLATAFORMA (super_admin)
 * — port 1:1 do `TeamBookingLinks` do CRM Vendus.
 *
 * Desacoplamento vs. o CRM de tenant original:
 *  - Vendedores via `usePlatformCrmSellerSlugs` (membros de squads da plataforma
 *    + atribuídos a leads). Nome/avatar vêm de `profiles` (só exibição), SEM
 *    `organization_id`.
 *  - Slug PERSISTIDO em `platform_crm_seller_booking` (`user_id` PK, `booking_slug`
 *    unique) — tabela `platform_crm_*` dedicada. NUNCA em `profiles.booking_slug`
 *    (fronteira tenant↔plataforma).
 */
...
export function PlatformCrmTeamBookingLinks() {
  const queryClient = useQueryClient();
  const { data: members, isLoading } = usePlatformCrmSellerSlugs();
  const [search, setSearch] = useState('');
  const [generatingFor, setGeneratingFor] = useState<string | null>(null);
  const [qrModal, setQrModal] = useState<QrModalState>({ open: false, url: '', name: '', slug: '' });
  ...
  const handleGenerateSlug = async (memberId: string, fullName: string | null) => {
    ...
    try {
      const baseSlug = generatePlatformCrmBookingSlug(fullName);
      // Verifica unicidade em platform_crm_seller_booking (slug unique)
      ...
      const { error } = await supabase
        .from('platform_crm_seller_booking')
        .upsert(
          { user_id: memberId, booking_slug: finalSlug },
          { onConflict: 'user_id' },
        );
      ...
      queryClient.invalidateQueries({ queryKey: ['platform-crm', 'seller-slugs'] });
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
                        ...
                        <div className="flex items-center gap-1 sm:gap-2 flex-wrap">
                          {!hasSlug ? (
                            <Button
                              size="sm"
                              variant="default"
                              onClick={() => handleGenerateSlug(member.id, member.full_name)}
      ...
      {/* Modal QR */}
      <Dialog open={qrModal.open} onOpenChange={(open) => setQrModal((s) => ({ ...s, open }))}>
