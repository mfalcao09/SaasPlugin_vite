...
import { FormBlock, FormBlockType, createFormBlock } from './platformFormTypes';
import { PlatformCrmFormBlockNode } from './PlatformCrmFormBlockNode';
import { ScrollArea } from '@/components/ui/scroll-area';
import { Button } from '@/components/ui/button';
import { Plus, Hand } from 'lucide-react';
import { cn } from '@/lib/utils';

// Porte de admin/forms/FormCanvas.tsx. Puro (sem tenant). Prefixo PlatformCrm.

interface PlatformCrmFormCanvasProps {
  formId: string;
  blocks: FormBlock[];
...
export function PlatformCrmFormCanvas({
  formId,
  blocks,
  selectedBlockId,
  onSelectBlock,
          ...
          {blocks.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-[500px] text-center">
              ...
              <div className="flex gap-3">
                <Button variant="outline" onClick={() => onAddBlock('welcome_screen')} className="gap-2">
                  <Hand className="h-4 w-4" />
                  Tela de Boas-vindas
                </Button>
                <Button onClick={() => onAddBlock('text')} className="gap-2">
