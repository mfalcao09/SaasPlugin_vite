...
import { Form, FormBlock, getBlockConfig, SelectOption, ScaleOptions } from './platformFormTypes';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { cn } from '@/lib/utils';
import { ChevronUp, Check, ArrowRight, Hand, Sparkles } from 'lucide-react';
import { PlatformCrmFormBlockMedia } from './PlatformCrmFormBlockMedia';
import { formButtonProps } from './PlatformCrmFormThemeWrapper';

// Porte de admin/forms/FormLivePreview.tsx. Puro (sem tenant). Prefixo PlatformCrm.

interface PlatformCrmFormLivePreviewProps {
  form: Form;
  blocks: FormBlock[];
  theme: 'light' | 'dark';
  onComplete?: () => void;
...
export function PlatformCrmFormLivePreview({
  form,
  blocks,
  theme,
  onComplete,
}: PlatformCrmFormLivePreviewProps) {
  ...
  const visibleBlocks = blocks.filter(
    (b) => !['conditional', 'score', 'tag', 'hidden_field'].includes(b.block_type),
  );

  const currentBlock = visibleBlocks[currentIndex];
  const progress =
    visibleBlocks.length > 0 ? ((currentIndex + 1) / visibleBlocks.length) * 100 : 0;
  ...
  }, [currentIndex, inputValue, handleNext, handlePrevious]);
  ...
  const renderBlockContent = (block: FormBlock) => {
    getBlockConfig(block.block_type);
    ...
    switch (block.block_type) {
      case 'welcome_screen':
        return (
          ...
          >
            ...
            <motion.div variants={staggerItemVariants}>
              <motion.div variants={buttonHoverVariants} initial="rest" whileHover="hover" whileTap="tap">
                <Button
                  ...
                  variant={btn.variant}
                  className={cn(
                    'gap-3 text-lg px-8 py-6 rounded-xl shadow-lg shadow-primary/30',
                    btn.className,
                  )}
                  onClick={handleNext}
                >
            ...
            <motion.p variants={staggerItemVariants} className={cn('mt-8 text-sm', themeStyles.muted)}>
              Pressione{' '}
              <kbd className="px-2 py-1 rounded bg-muted text-xs font-mono">Enter ↵</kbd> para continuar
            </motion.p>
          </motion.div>
        );
                  ...
                  return (
                    <motion.button
                      ...
                      className={cn(
                        ...
                        isSelected ? 'border-primary bg-primary/10' : themeStyles.optionHover,
                      )}
                    >
      ...
      case 'divider':
        return (
          ...
          >
            <motion.div variants={staggerItemVariants}>
              <PlatformCrmFormBlockMedia block={block} />
            </motion.div>
  ...
  return (
    <div className={cn('min-h-[500px] flex flex-col relative overflow-hidden', themeStyles.bg)}>
      ...
      <div className={cn('absolute bottom-4 left-0 right-0 flex justify-center', themeStyles.muted)}>
        <motion.div
