...
} from './PlatformCrmFormThemeWrapper';
import { PlatformCrmFormThemePresetsSection } from './PlatformCrmFormThemePresetsSection';

/**
 * Porte de admin/forms/FormDesignPanel.tsx.
 * Adaptações: useAuth REMOVIDO. Logo upload em `form-media` com path
 * `platform/{form_id}/logo-*.{ext}` (sem organization_id), URL assinada longa.
 * NOTA: bucket `form-media` pode ser asset do tenant — sinalizado no relatório.
 */

interface PlatformCrmFormDesignPanelProps {
  form: Form;
...
export function PlatformCrmFormDesignPanel({ form, onUpdateTheme }: PlatformCrmFormDesignPanelProps) {
  ...
  const handleLogoUpload = async (file: File) => {
    ...
    try {
      ...
      const path = `platform/${form.id}/logo-${Date.now()}.${ext}`;
      const { error } = await supabase.storage
        .from('form-media')
        .upload(path, file, { cacheControl: '3600', upsert: true });
      if (error) throw error;
      // Bucket pode ser privado — usar URL assinada de longa duração (10 anos)
    ...
    <aside className="w-full md:w-80 shrink-0 md:border-l bg-card flex flex-col h-full">
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-6">
          {/* Temas prontos */}
          <PlatformCrmFormThemePresetsSection
            currentTheme={theme}
          ...
          <section className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              <Label htmlFor="progress-show" className="text-sm">
                Exibir
              </Label>
              <Switch
                id="progress-show"
              ...
              <>
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v: FormProgressPosition) =>
                      onUpdateTheme({ progress_position: v })
                    }
                  >
                    <SelectTrigger className="h-9">
                      <SelectValue />
          ...
          <section className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={theme.font_family} onValueChange={(v) => onUpdateTheme({ font_family: v })}>
                <SelectTrigger className="h-9">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v: FormLetterSpacing) => onUpdateTheme({ heading_letter_spacing: v })}
              >
                <SelectTrigger className="h-9">
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="flex items-center justify-between pt-1">
              <Label htmlFor="heading-upper" className="text-sm">
                Títulos em CAIXA ALTA
              </Label>
              <Switch
                id="heading-upper"
                checked={theme.heading_transform === 'uppercase'}
                onCheckedChange={(v) => onUpdateTheme({ heading_transform: v ? 'uppercase' : 'none' })}
              />
            </div>
          </section>
            ...
            <div className="space-y-2">
              <div className="space-y-1.5">
                ...
                <ColorField
                  ...
                  onChange={(v) => onUpdateTheme({ primary_color: v })}
                />
              </div>
              <div className="space-y-1.5">
                ...
                <ColorField
                  ...
                  onChange={(v) => onUpdateTheme({ background_color: v })}
                />
              </div>
              <div className="space-y-1.5">
                ...
                <ColorField
                  ...
                  onChange={(v) => onUpdateTheme({ text_color: v })}
                />
              </div>
            </div>
          </section>
