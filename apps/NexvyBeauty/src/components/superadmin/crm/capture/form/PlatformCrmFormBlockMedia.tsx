...
import { FormBlock, toEmbedUrl } from './platformFormTypes';
import { cn } from '@/lib/utils';
import { ChevronLeft, ChevronRight, Image as ImageIcon } from 'lucide-react';

// Porte de admin/forms/FormBlockMedia.tsx. Puro (sem tenant). Prefixo PlatformCrm.

interface PlatformCrmFormBlockMediaProps {
  block: FormBlock;
  className?: string;
}
 ...
 */
export function PlatformCrmFormBlockMedia({ block, className }: PlatformCrmFormBlockMediaProps) {
  ...
  if (block.block_type === 'image') {
    ...
    return (
      <div className={cn('w-full max-w-xl mx-auto px-6', className)}>
        ...
        {block.label && (
          <p className="text-center text-sm text-muted-foreground mt-3">{block.label}</p>
        )}
      </div>
    );
  }
        ...
        <div className={cn('w-full max-w-xl mx-auto px-6', className)}>
          <div className="flex items-center justify-center h-48 rounded-xl border-2 border-dashed bg-muted/30 text-muted-foreground">
            <p className="text-sm text-center">
              Cole a URL do YouTube, Vimeo ou Loom no editor.
              <br />
