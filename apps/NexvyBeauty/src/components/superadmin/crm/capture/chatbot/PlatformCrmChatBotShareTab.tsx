...
import type { FunnelChannelConfig } from '@/types/funnel';
...
export function PlatformCrmChatBotShareTab({ funnel }: Props) {
  const [copied, setCopied] = useState<string | null>(null);
  const { data: baseUrl } = usePublicAppUrl();
  const editor = typeof window !== 'undefined' && isEditorHost();

  // channels é Json na Row de plataforma — cast para a config rica só p/ leitura.
  const channels = (funnel.channels ?? {}) as unknown as Partial<FunnelChannelConfig>;
  const slug = channels.chat?.slug_override || funnel.slug;
