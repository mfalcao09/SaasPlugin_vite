...
import { PlatformCrmImageUploadField } from './PlatformCrmImageUploadField';
...
interface Props {
  ...
  /** Escopo do Storage (path `platform/{funnelId}/...`) — sem organization_id. */
  funnelId?: string;
}

export function PlatformCrmAppearanceForm({ channel, appearance, onChange, funnelId }: Props) {
    ...
    <Accordion type="multiple" defaultValue={['colors', 'typography', 'layout']} className="space-y-2">
      ...
      <AccordionItem value="channel" className="border rounded-lg px-3">
        ...
        <AccordionContent className="space-y-3 pt-2">
          ...
          {channel === 'form' && <FormOptionsFields opts={appearance.channel_options as FormChannelOptions} setOpt={setOpt} funnelId={funnelId} />}
          {channel === 'widget' && <WidgetOptionsFields opts={appearance.channel_options as WidgetChannelOptions} setOpt={setOpt} />}
          {channel === 'quiz' && <QuizOptionsFields opts={appearance.channel_options as QuizChannelOptions} setOpt={setOpt} funnelId={funnelId} />}
        </AccordionContent>
      </AccordionItem>
...
function FormOptionsFields({ opts, setOpt, funnelId }: { opts: FormChannelOptions; setOpt: (p: any) => void; funnelId?: string }) {
  return (
    <>
      <div className="space-y-1.5">
        <Label>Layout</Label>
      ...
      <PlatformCrmImageUploadField label="Imagem lateral (split-screen)" value={opts.side_image_url} onChange={(url) => setOpt({ side_image_url: url })} folder="form-side" funnelId={funnelId} />
    </>
  );
}
...
function QuizOptionsFields({ opts, setOpt, funnelId }: { opts: QuizChannelOptions; setOpt: (p: any) => void; funnelId?: string }) {
  return (
    <>
      <div className="space-y-1.5">
      ...
      <PlatformCrmImageUploadField label="Imagem da tela de resultado" value={opts.result_image_url} onChange={(url) => setOpt({ result_image_url: url })} folder="quiz-result" funnelId={funnelId} />
      <div className="space-y-1.5">
