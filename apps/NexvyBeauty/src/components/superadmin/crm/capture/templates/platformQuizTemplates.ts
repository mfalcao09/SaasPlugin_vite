/**
 * CRM de PLATAFORMA (super_admin) — SEED estático da biblioteca de Templates de Quiz.
 *
 * Porte FIEL (1:1) de `src/data/quizTemplates.ts` (tenant/Vendus) para o namespace
 * platform, garantindo que a tela de Templates NUNCA nasça vazia (o gallery de DB —
 * `platform_crm_quiz_templates` — começa sem linhas). O bundle da plataforma passa a
 * ser dono do seu próprio seed, sem acoplar ao runtime do tenant (só depende dos
 * tipos compartilhados `@/types/funnel`, que são app-wide).
 *
 * Conteúdo idêntico à fonte (parity-check: mesmo número de templates/blocos). A única
 * diferença é este cabeçalho de proveniência.
 */
