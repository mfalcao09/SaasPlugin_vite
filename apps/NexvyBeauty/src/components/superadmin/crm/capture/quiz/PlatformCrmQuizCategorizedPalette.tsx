...
/**
 * CRM de PLATAFORMA (super_admin) — paleta categorizada do QuizBuilder, DESACOPLADA do tenant.
 * Componente 100% puro (types/lib/ui neutros) — porte 1:1 de
 * `admin/capture/quiz/QuizCategorizedPalette.tsx`, apenas renomeado com prefixo PlatformCrm.
 */

export interface PlatformCrmQuizPaletteItem {
...
export interface PlatformCrmQuizPaletteCategory {
  id: string;
  label: string;
  emoji: string;
  accent: string; // tailwind text color class
  bg: string;     // tailwind bg class
  items: PlatformCrmQuizPaletteItem[];
}

export const PLATFORM_CRM_QUIZ_PALETTE: PlatformCrmQuizPaletteCategory[] = [
  {
    id: 'questions',
    label: 'Perguntas',
    emoji: '📝',
...
interface Props {
  onAddBlock: (item: PlatformCrmQuizPaletteItem) => void;
}

export const PlatformCrmQuizCategorizedPalette = memo(function PlatformCrmQuizCategorizedPalette({ onAddBlock }: Props) {
  const handleDragStart = (e: React.DragEvent, item: PlatformCrmQuizPaletteItem) => {
    ...
    <div className="h-full flex flex-col bg-card">
      ...
      <ScrollArea className="flex-1">
        <div className="p-3 space-y-5">
          {PLATFORM_CRM_QUIZ_PALETTE.map((cat) => (
...
export function applyPlatformCrmQuizPresetToBlockData(
