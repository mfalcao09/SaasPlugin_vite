...
export default function Dashboard() {
  ...
  const stats = [
    ...
    { label: 'Receita Confirmada', value: formatCurrency(receitaMes), icon: DollarSign, color: 'text-green-400' },
    { label: 'Orçamentos', value: String(orcamentos?.length ?? 0), icon: FileText, color: 'text-purple-400' },
  ]
  ...
  return (
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-xl">
        ...
        <div className="divide-y divide-slate-700">
          {ordensAbertas.length === 0 ? (
            <p className="px-5 py-8 text-center text-slate-500 text-sm">Nenhuma OS aberta no momento.</p>
          ) : (
      ...
      <button
        onClick={() => navigate('/')}
        className="w-full bg-orange-600/10 border border-orange-600/30 rounded-xl p-5 flex items-center gap-4 hover:bg-orange-600/15 transition-colors text-left"
      >
        <ArrowLeft className="h-8 w-8 text-orange-400 shrink-0" />
        <div>
          <p className="font-semibold text-white">CRM de Vendas</p>
          <p className="text-sm text-slate-400">Voltar ao painel de leads, cadências e conversas.</p>
        </div>
      </button>
    </div>
    </OficinaLayout>
