...
- ⏸️ **L13 ADIADO** (investigado: `followup-ai-draft` NÃO é coberto por `platform-sales-copilot` — contratos diferentes; feature dropada no porte → precisa edge+UI de strategy+eyeball). **L8 migration ✅** (`platform_crm_lead_transfer_history` aplicada via MCP + SQL no repo + types regen 18630) — **falta só a UI** (modal + histórico na aba **wallet/Carteira** do `PlatformCrmLeadDetail`, usando `usePlatformCrmSellers`+`usePlatformCrmSquads` → insert history + update lead `assigned_to`/`squad_id`/`previous_assigned_to`).
- **Próximo:** **L8 UI** → **L1–L4/L10** (edges+crons OUTBOUND: crons AO VIVO + idempotência + guard 0-regras=0-envio + LOG de cada envio — **verificar o guard em cada dispatcher ANTES de ligar o cron**) → **P6** (D2/D4/D5/D7/D8/D9-b). P5 Meta + P7 Utmify adiados sem creds no `.env`. **NÃO tocar** módulo-por-host ERP↔CRM. Relatório noturno `.md`+`.html` ao fim.
