...
- **Próximo = P4 (LOTE L1–L13)** — ports/crons product-aware. **Crons ao vivo** (decisão noturna) com idempotência + guard 0-regras=0-envio + log de cada envio. Dep f2-liveness era do browser (bloqueado), não gate real → sigo. **NÃO tocar** diferenciação módulo-por-host ERP↔CRM (never-touch-alone).
