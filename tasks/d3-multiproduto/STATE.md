...
- ⏸️ **L13 ADIADO** (investigado: `followup-ai-draft` NÃO é coberto por `platform-sales-copilot` — contratos diferentes; feature dropada no porte → precisa edge+UI de strategy+eyeball). **L8 ✅ COMPLETO** (migration `platform_crm_lead_transfer_history` + UI: `PlatformCrmLeadTransfer.tsx` modal + histórico fiados no LeadWalletTab; deploy `index-B_lsrKUR`, gate verde). Eyeball do modal = pendente Chrome.
- ✅ **Crons L2-L3 LIVE** (jobid 20 `platform-campaign-dispatcher` 1/min + jobid 21 `platform-auto-notifications` */15; guards verificados ANTES, dormentes até config — active_campaigns=0/settings=0; run `succeeded` provado no job_run_details). ⏸️ **L1/L4/L10 ADIADOS** (edges platform inexistentes: campaign-recurring-snapshot/booking-dispatcher/manual-outreach-batch — portar a edge antes). **✅ P4 LOTE COMPLETO** (L12/L6/L7/L11/L8/L2-L3 feitos; L5/L13/L1/L4/L10 adiados-com-razão).
- **✅ P6 triado:** **D8** feito (booking conversacional: migration `booking_experience` + campo no editor; deploy `index-BqittP2d`). **D2/D4/D5/D7/D9 adiados** (UI profunda / decisão UX / stack push inexistente — razões no feature-list).
- **🏁 ROADMAP AUTÔNOMO COMPLETO** (2026-07-03). Tudo que era autônomo-fazível está feito, gated, deployado e provado. **Relatório noturno:** `RELATORIO-NOTURNO-2026-07-03.md`+`.html`. **Loop encerra aqui** — o que resta precisa de Marcelo: (1) **eyeball Chrome** (prints visuais); (2) **creds Meta (P5) + Utmify (P7)** no `.env`; (3) **sessão conjunta** módulo-por-host ERP↔CRM (never-touch-alone). (D2/D4/D5/D7/D8/D9-b). P5 Meta + P7 Utmify adiados sem creds no `.env`. **NÃO tocar** módulo-por-host ERP↔CRM. Relatório noturno `.md`+`.html` ao fim.
