...
- **P3 EM PROGRESSO** (Marcelo autorizou DNS + "os dois em paralelo", 2026-07-03). **Track B (infra):** DNS `gestao.nexvy.tech` A → `145.223.29.96` APLICADO (Hostinger, overwrite:false, apex 2.57.91.91 e mail intactos; propagando TTL 300). Router Traefik `nexvy-gestao-grupo.yml` no VPS → `nexvy-beauty-svc` (mesmo container; YAML OK, hot-reload). **Falta B:** (1) ensinar o SPA que `gestao.nexvy.tech` é host de gestão — `HostConfinementGuard` (allowlist) + host-class/`isGestaoHostname` (7 arquivos usam host-logic) → deploy; (2) verificar cert LE + `https://gestao.nexvy.tech` servindo o CRM. **Track A (re-skin):** ref canônico = `~/OneDrive/ARQUIVOS CLAUDE/nexvy-design-export/` (BRAND-EXPLAINER.md + tokens.json + brand-visual-guide.html) + `nexvy-rebrand/` (logos v6). Próximo A: ler tokens → propor direção institucional → aplicar tema. **NÃO tocar** diferenciação módulo-por-host (grupo-CRM vs ERP-beauty) — é never-touch-alone (ERP fala com tenant).
