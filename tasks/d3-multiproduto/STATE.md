...
- **P4 EM PROGRESSO (LOTE).** ✅ **L12** (removidos 3 UI órfãos; agent_configs NÃO era órfão — mantido) · ✅ **L6** (aba Captação: useEffect resync tab←initialTab). Commit `6f16aec`, gates verdes. ⏸️ **L5 ADIADO** (ProviderCooldownBadge lê `evolution_instances` = tenant → fronteira; fazer com painel de conexões da plataforma).
- **Próximo LOTE (turno fresco):** **L7** (enroll/stop de cadência no LeadDetail — edges `platform-cadence-enroll/stop/tick/on-response` JÁ existem; LeadDetail em `crm/leads/PlatformCrmLeadsManager.tsx`) · **L11** (reusar `crm/campaigns/ContextLibrary.tsx` no `CadenceWizard`) · **L8** (transfer: migration + modal) · **L1–L4/L10** (edges+crons OUTBOUND: ao vivo + idempotência + guard 0-regras + LOG cada envio) · **L13** (followup-ai-draft: checar se já coberto). Deploy dos itens de UI batchado (evita rebuild por item). Depois **P6**. **NÃO tocar** módulo-por-host ERP↔CRM.
